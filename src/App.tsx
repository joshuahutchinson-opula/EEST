...
const GCT_RATE = 0.15;
const CURRENT_USER = { name: "Marcus Webb", initials: "MW", color: "#3b82f6", email: "marcus@etechsystems.com" };
...
function makeFmt(currency: "USD" | "JMD") {
  return (usdAmt: number, compact = false): string => {
    const amt = currency === "JMD" ? usdAmt * JMD_RATE : usdAmt; const sym = currency === "JMD" ? "J$" : "$";
    ...
    <div draggable onDragStart={(e) => { e.stopPropagation(); onDragStart(e, project.id); }} onDragEnd={onDragEnd} onClick={onClick} className={clsx("group relative rounded-2xl cursor-pointer select-none transition-all duration-200", isDragging ? "opacity-25 scale-[0.96]" : "md:hover:-translate-y-1")} style={{ background: "rgba(255,255,255,0.055)", backdropFilter: "blur(24px)", border: isDragging ? "1px solid rgba(255,255,255,0.05)" : "1px solid rgba(255,255,255,0.11)", borderLeft: `3px solid ${column.color}`, boxShadow: isDragging ? "none" : "0 2px 16px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.09)" }}>
      <div className="p-3 md:p-4 md:pl-5">
        ...
        <div className="flex items-center flex-wrap gap-1.5 mb-2">
          ...
          <span className={clsx("text-[9px] md:text-[10px] font-bold px-1.5 py-0.5 rounded-full uppercase tracking-wide", project.risk === "high" ? "bg-rose-500/20 text-rose-400 border border-rose-500/25" : project.risk === "medium" ? "bg-amber-500/20 text-amber-400 border border-amber-500/25" : "bg-emerald-500/20 text-emerald-400 border border-emerald-500/25")}>{project.risk}</span>
        </div>
        ...
        <div className="flex items-center justify-between pt-2.5" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center gap-1.5">
            ...
            {project.collaborators && project.collaborators.length > 0 && <span className="text-[#484f58] text-[10px]">+{project.collaborators.length}</span>}
          </div>
...
function Dashboard({ navigate }: { navigate: (p: Page) => void }) {
  ...
  const handleDrop = async (colId: Stage) => {
    if (dragging) {
      ...
      try { await API.projects.update(dragging, { stage: colId }); } catch { toast.error("Failed to update stage"); }
    }
  ...
  const handleUpdate = async (updated: Project) => { setProjects((prev) => prev.map((p) => p.id === updated.id ? updated : p)); setSelectedDeal(updated); try { await API.projects.update(updated.id, updated); toast.success("Project updated"); } catch { fetchProjects(); } };
  ...
  if (loading) return <div className="px-3 md:px-5 py-4 md:py-6 space-y-4"><Skeleton className="h-10 w-48" /><div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-3">{[1,2,3,4].map(i => <Skeleton key={i} className="h-24 rounded-2xl" />)}</div><div className="flex gap-2 md:gap-3"><Skeleton className="h-64 w-[260px] rounded-2xl" /><Skeleton className="h-64 w-[260px] rounded-2xl" /></div></div>;

  return (
    <div>
      {selectedDeal && selectedColumn && <DealModal project={selectedDeal} column={selectedColumn} onClose={() => setSelectedDeal(null)} navigate={navigate} onUpdate={handleUpdate} />}
...
function NewProjectModal({ onClose, onAdd }: { onClose: () => void; onAdd: (p: Project) => void }) {
  ...
  const [collabName, setCollabName] = useState(""); const [collabRole, setCollabRole] = useState(""); const [collaborators, setCollaborators] = useState<{ name: string; initials: string; color: string; role: string }[]>([]);
  const canSubmit = name.trim() && client.trim();

  const addCollaborator = () => { if (!collabName.trim()) return; const initials = collabName.split(" ").map(n => n[0]).join("").slice(0, 2).toUpperCase(); const colors = ["#10b981","#f59e0b","#06b6d4","#f97316","#ec4899"]; setCollaborators((prev) => [...prev, { name: collabName.trim(), initials, color: colors[prev.length % colors.length], role: collabRole.trim() || "Team Member" }]); setCollabName(""); setCollabRole(""); };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}><div className="absolute inset-0" style={{ background: "rgba(0,0,0,0.65)", backdropFilter: "blur(8px)" }} /><motion.div initial={{ opacity: 0, scale: 0.94, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: "spring", damping: 26, stiffness: 360 }} onClick={(e) => e.stopPropagation()} className="relative z-10 w-full max-w-[600px] max-h-[90vh] overflow-y-auto rounded-3xl" style={{ background: "rgba(7,12,26,0.92)", backdropFilter: "blur(52px) saturate(200%)", border: "1px solid rgba(255,255,255,0.13)", boxShadow: "0 32px 80px rgba(0,0,0,0.9), inset 0 1px 0 rgba(255,255,255,0.12)" }}>
      <div className="flex items-center justify-between px-5 md:px-7 pt-5 md:pt-7 pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}><div><h2 className="text-white text-[1rem] md:text-[1.1rem] font-bold tracking-tight">New Project</h2><p className="text-[#8b949e] text-[12px] mt-0.5">Account Owner: {CURRENT_USER.name}</p></div><button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-white/[0.08] transition-colors cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px]" style={{ border: "1px solid rgba(255,255,255,0.10)" }}><X className="w-4 h-4 text-[#8b949e]" /></button></div>
...
function DealModal({ project, column, onClose, navigate, onUpdate }: { project: Project; column: Column; onClose: () => void; navigate: (p: Page) => void; onUpdate: (p: Project) => void }) {
  ...
  const handleSave = async () => {
    setSaving(true);
    const updated: Project = { ...project, name: editName, client: editClient, location: editLocation, value: parseFloat(editValue) || project.value, risk: editRisk, dueDate: editDueDate, summary: editSummary || undefined, notes: editNotes || undefined };
    onUpdate(updated); setEditing(false); setSaving(false);
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}><div className="absolute inset-0" style={{ background: "rgba(0,0,0,0.65)", backdropFilter: "blur(8px)" }} /><motion.div initial={{ opacity: 0, scale: 0.93, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ type: "spring", damping: 26, stiffness: 360 }} onClick={(e) => e.stopPropagation()} className="relative z-10 w-full max-w-[540px] max-h-[90vh] overflow-y-auto rounded-3xl" style={{ background: "rgba(7,12,26,0.78)", backdropFilter: "blur(52px) saturate(200%)", border: "1px solid rgba(255,255,255,0.13)", boxShadow: "0 32px 80px rgba(0,0,0,0.9), inset 0 1px 0 rgba(255,255,255,0.12)" }}>
      <div className="relative px-5 md:px-7 pt-5 md:pt-7 pb-5">
        <div className="flex items-start justify-between gap-4"><div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-1.5 mb-2.5"><span className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-[0.14em] px-2.5 py-1 rounded-full" style={{ background: `${column.color}22`, color: column.color, border: `1px solid ${column.color}44` }}><span className="w-1.5 h-1.5 rounded-full" style={{ background: column.color }} />{column.label}</span>{ls && <span className="text-[9px] font-bold px-2 py-0.5 rounded-full" style={{ background: ls.bg, color: ls.text }}>{project.leadSource}</span>}<span className={clsx("text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full", project.risk === "high" ? "bg-rose-500/20 text-rose-400" : project.risk === "medium" ? "bg-amber-500/20 text-amber-400" : "bg-emerald-500/20 text-emerald-400")}>{project.risk} risk</span></div>
          {editing ? <input value={editName} onChange={(e) => setEditName(e.target.value)} className="text-white text-[1.1rem] font-bold bg-transparent border-b border-blue-500 w-full focus:outline-none" /> : <h2 className="text-white text-[1rem] md:text-[1.1rem] font-bold leading-snug tracking-tight">{project.name}</h2>}
        ...
        <div className="flex gap-2"><button onClick={() => setEditing(!editing)} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-white/[0.08] transition-colors cursor-pointer active:scale-[0.97] transition-transform" style={{ border: "1px solid rgba(255,255,255,0.10)" }}><Pencil className="w-4 h-4 text-[#8b949e]" /></button><button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-white/[0.08] transition-colors cursor-pointer active:scale-[0.97] transition-transform" style={{ border: "1px solid rgba(255,255,255,0.10)" }}><X className="w-4 h-4 text-[#8b949e]" /></button></div></div>
        {editing && <div className="grid grid-cols-2 gap-2 mt-3"><div><input value={editLocation} onChange={(e) => setEditLocation(e.target.value)} placeholder="Location" className="w-full h-8 rounded-xl px-2 text-[12px] bg-transparent text-white" style={G.input} /></div><div><input type="number" value={editValue} onChange={(e) => setEditValue(e.target.value)} placeholder="Value" className="w-full h-8 rounded-xl px-2 text-[12px] bg-transparent text-white" style={G.input} /></div><div><select value={editRisk} onChange={(e) => setEditRisk(e.target.value as "low"|"medium"|"high")} className="w-full h-8 rounded-xl px-2 text-[12px] bg-transparent text-white" style={G.input}>{["low","medium","high"].map((r) => <option key={r} value={r}>{r}</option>)}</select></div><div><input type="date" value={editDueDate} onChange={(e) => setEditDueDate(e.target.value)} className="w-full h-8 rounded-xl px-2 text-[12px] bg-transparent text-white" style={{ ...G.input, colorScheme: "dark" }} /></div></div>}
    ...
    <div className="px-3 md:px-5 py-4 md:py-6">
      ...
      {studioView === "canvas" ? (
        <div className="rounded-2xl overflow-hidden relative" style={{ ...G.card, minHeight: "50vh" }}>
          ...
          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between pointer-events-none"><div className="px-3 py-1.5 rounded-xl text-[10px] md:text-[11px] font-semibold text-white pointer-events-auto" style={G.liquidGlass}>Floor Plan View</div><button onClick={() => navigate("design-canvas")} className="px-3 py-1.5 rounded-xl text-[10px] md:text-[11px] font-bold text-white pointer-events-auto cursor-pointer active:scale-[0.97] transition-transform" style={{ background: "#3b82f6" }}>Open Full Canvas</button></div>
        </div>
      ) : (
        <>
          <div className="flex items-center gap-2 mb-4 md:mb-5 flex-wrap">{stageFilters.map((f) => (<button key={f.id} onClick={() => setFilter(f.id)} className={clsx("h-7 px-3 rounded-full text-[11px] md:text-[12px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform", filter === f.id ? "text-white" : "text-[#8b949e]")} style={filter === f.id ? { background: "#3b82f6", boxShadow: "0 2px 12px rgba(59,130,246,0.3)" } : G.subtle}>{f.label}</button>))}<div className="relative ml-1"><Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3 h-3 text-[#484f58]" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search…" className="h-7 rounded-xl pl-7 pr-3 text-[11px] md:text-[12px] text-[#e6edf3] placeholder:text-[#484f58] focus:outline-none w-36 md:w-44 transition-all" style={G.input} /></div><span className="text-[#484f58] text-[11px] md:text-[12px] ml-1">{filtered.length} projects</span></div>
          {filtered.length === 0 ? <EmptyState icon={Layers} title="No projects found" description={search ? "Try adjusting your search." : "Create a new project."} /> : viewMode === "grid" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">{filtered.map((project) => { const badge = stageBadge(project.stage); const hasDesign = ["design","proposal","negotiation","win"].includes(project.stage); return (<div key={project.id} className="group rounded-2xl overflow-hidden cursor-pointer transition-all duration-200 md:hover:-translate-y-1 relative" style={{ ...G.card }}><div className="relative h-[100px] md:h-[112px] bg-[#070c1a]" onClick={() => { setActiveProjectId(project.id); navigate("project-detail"); }}><MiniFloorPlan project={project} />{hasDesign && <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2" style={{ background: "rgba(0,0,0,0.5)", backdropFilter: "blur(4px)" }}><button onClick={(e) => { e.stopPropagation(); setActiveProjectId(project.id); navigate("design-canvas"); }} className="h-7 px-3 rounded-xl text-white text-[11px] font-bold cursor-pointer active:scale-[0.97] transition-transform" style={{ background: "#3b82f6" }}><Eye className="w-3 h-3" /> Open</button></div>}<div className={clsx("absolute top-2 right-2 text-[10px] font-bold px-2 py-0.5 rounded-full", badge.cls)}>{badge.label}</div></div><div className="p-3 md:p-4" onClick={() => { setActiveProjectId(project.id); navigate("project-detail"); }}><h3 className="text-white text-[12px] md:text-[13px] font-semibold leading-snug mb-1 line-clamp-1">{project.name}</h3><p className="text-[#8b949e] text-[10px] md:text-[11px] font-medium mb-2 flex items-center gap-1"><Building2 className="w-3 h-3" /> {project.client}</p><div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="flex items-center gap-1 text-[#484f58] text-[10px]"><Camera className="w-3 h-3" />{project.cameras}</span><span className="flex items-center gap-1 text-[#484f58] text-[10px]"><Fingerprint className="w-3 h-3" />{project.devices}</span></div><div className="w-5 h-5 rounded-full flex items-center justify-center text-[9px] font-bold text-white" style={{ background: project.assignee.color }}>{project.assignee.initials}</div></div></div></div>); })}</div>
          ) : (
            <div className="rounded-2xl overflow-hidden" style={G.card}><div className="overflow-x-auto"><div className="grid gap-3 md:gap-4 px-3 md:px-4 py-2.5" style={{ gridTemplateColumns: "2fr 1fr 80px 80px 100px 80px 32px", minWidth: "700px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>{["Project","Client","Cameras","Devices","Stage","Value",""].map((h) => (<span key={h} className="text-[#484f58] text-[10px] font-bold uppercase tracking-widest">{h}</span>))}</div>{filtered.map((project) => { const badge = stageBadge(project.stage); return (<div key={project.id} className="grid gap-3 md:gap-4 px-3 md:px-4 py-3.5 items-center transition-all group hover:bg-white/[0.03]" style={{ gridTemplateColumns: "2fr 1fr 80px 80px 100px 80px 32px", minWidth: "700px", borderBottom: "1px solid rgba(255,255,255,0.04)" }}><div className="min-w-0 cursor-pointer" onClick={() => { setActiveProjectId(project.id); navigate("project-detail"); }}><p className="text-white text-[12px] md:text-[13px] font-semibold truncate">{project.name}</p><p className="text-[#484f58] text-[10px] truncate">{project.location}</p></div><p className="text-[#8b949e] text-[11px] md:text-[12px] truncate font-medium">{project.client}</p><p className="text-[#8b949e] text-[11px] flex items-center gap-1"><Camera className="w-3 h-3 text-[#484f58]" />{project.cameras}</p><p className="text-[#8b949e] text-[11px] flex items-center gap-1"><Fingerprint className="w-3 h-3 text-[#484f58]" />{project.devices}</p><span className={clsx("text-[10px] font-bold px-2 py-0.5 rounded-full w-fit", badge.cls)}>{badge.label}</span><p className="text-white text-[12px] md:text-[13px] font-bold">{fmt(project.value, true)}</p><button onClick={() => handleDelete(project.id)} className="w-6 h-6 rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-rose-500/20 cursor-pointer active:scale-[0.97] transition-transform"><Trash2 className="w-3 h-3 text-rose-400" /></button></div>); })}</div></div>
          )}
        </>
      )}
    </div>
...
function ProjectDetail({ navigate }: { navigate: (p: Page) => void }) {
  ...
  return (
    <div className="px-3 md:px-5 py-4 md:py-6 max-w-[1200px]">
      ...
      {activeTab === "overview" && (<div className="grid grid-cols-1 md:grid-cols-3 gap-4"><div className="md:col-span-2 space-y-4"><div className="rounded-2xl p-4 md:p-5" style={G.card}><h3 className="text-white text-[13px] md:text-[14px] font-bold mb-3">Project Scope</h3><p className="text-[#8b949e] text-[12px] md:text-[13px] leading-relaxed">{p.summary ?? "No scope defined yet."}</p></div><div className="rounded-2xl p-4 md:p-5" style={G.card}><h3 className="text-white text-[13px] md:text-[14px] font-bold mb-4">Project Team</h3><div className="space-y-3"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-xl flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0" style={{ background: p.assignee.color, boxShadow: `0 0 12px ${p.assignee.color}44` }}>{p.assignee.initials}</div><div><p className="text-white text-[12px] md:text-[13px] font-semibold">{p.assignee.name}</p><p className="text-[#8b949e] text-[10px] md:text-[11px]">Account Owner</p></div></div>{p.collaborators?.map((c) => (<div key={c.name} className="flex items-center gap-3"><div className="w-8 h-8 rounded-xl flex items-center justify-center text-[11px] font-bold text-white flex-shrink-0" style={{ background: c.color }}>{c.initials}</div><div><p className="text-white text-[12px] md:text-[13px] font-semibold">{c.name}</p><p className="text-[#8b949e] text-[10px] md:text-[11px]">{c.role}</p></div></div>))}</div></div></div><div className="space-y-4"><div className="rounded-2xl p-4 md:p-5" style={G.card}><h3 className="text-white text-[13px] md:text-[14px] font-bold mb-4">Timeline</h3><div className="space-y-2">{COLUMNS.filter(c => !["win","lose"].includes(c.id)).map((col) => { const entry = stageHistory.find((s) => s.stage === col.id); const isPast = COLUMNS.indexOf(col) < COLUMNS.indexOf(COLUMNS.find((c) => c.id === p.stage)!); const isCurrent = col.id === p.stage; return (<div key={col.id} className="flex items-center gap-3"><div className={clsx("w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0", isPast ? "bg-emerald-500/20" : isCurrent ? "bg-blue-500/20 ring-2 ring-blue-500/40" : "bg-white/[0.04]")}>{isPast ? <CheckCircle2 className="w-3 h-3 text-emerald-400" /> : isCurrent ? <Clock className="w-3 h-3 text-blue-400" /> : <div className="w-1.5 h-1.5 rounded-full bg-white/20" />}</div><div className="flex-1 flex items-center justify-between"><span className={clsx("text-[11px] md:text-[12px] font-semibold", isPast ? "text-[#8b949e]" : isCurrent ? "text-white" : "text-[#484f58]")}>{col.label}</span><span className="text-[#484f58] text-[10px] md:text-[11px]">{entry?.date || "—"}</span></div></div>); })}</div></div></div></div>)}
      {activeTab === "quotes" && (quotes.length === 0 ? <EmptyState icon={DollarSign} title="No workbooks yet" description="Create a workbook for this project." action={{ label: "Open Workbook", onClick: () => navigate("workbook") }} /> : <div className="space-y-3">{quotes.map((q) => (<div key={q.id} className="flex items-center justify-between rounded-2xl p-4" style={G.card}><div className="flex items-center gap-4"><div className="w-9 h-9 rounded-xl flex items-center justify-center" style={{ background: "rgba(59,130,246,0.15)" }}><DollarSign className="w-4 h-4 text-blue-400" /></div><div><p className="text-white text-[13px] font-semibold">{q.refNumber}</p><p className="text-[#484f58] text-[11px]">{q.date} · {q.status}</p></div></div><button onClick={() => navigate("workbook")} className="h-8 px-3 rounded-xl text-[#8b949e] text-[12px] font-semibold hover:text-white cursor-pointer active:scale-[0.97] transition-transform" style={G.btn}>Open</button></div>))}</div>)}
...
const CANVAS_TOOLS = [
  ...
  { id: "ptz", icon: RotateCcw, label: "PTZ" }, { id: "door", icon: DoorOpen, label: "Door" }, { id: "access", icon: Key, label: "Reader" },
  { id: "panel", icon: PanelRight, label: "Panel" }, { id: "power", icon: Zap, label: "Power" }, { id: "server", icon: Server, label: "NVR" },
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "#070c1a" }}>
      <header className="h-12 flex items-center gap-2 md:gap-4 px-3 md:px-4 flex-shrink-0 z-40" style={G.liquidGlass}>
        <button onClick={() => navigate("design-studio")} className="flex items-center gap-1.5 text-[#8b949e] hover:text-white text-[11px] md:text-[12px] font-semibold transition-colors flex-shrink-0 cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"><ArrowLeft className="w-3.5 h-3.5" /><span className="hidden md:inline">Back</span></button>
        <div className="flex-1" />
        <button onClick={() => setView3D(!view3D)} className={clsx("flex items-center gap-1.5 h-7 px-2 md:px-3 rounded-xl text-[10px] md:text-[11px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform", view3D ? "text-violet-400" : "text-[#8b949e]")} style={view3D ? { background: "rgba(139,92,246,0.15)" } : G.btn}><Box className="w-3 h-3" /> {view3D ? "2D" : "3D"}</button>
        <button onClick={() => setShowFov(!showFov)} className={clsx("flex items-center gap-1.5 h-7 px-2 md:px-3 rounded-xl text-[10px] md:text-[11px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform", showFov ? "text-blue-400" : "text-[#8b949e]")} style={showFov ? { background: "rgba(59,130,246,0.15)" } : G.btn}><Eye className="w-3 h-3" /> FOV</button>
...
function Workbook({ navigate }: { navigate: (p: Page) => void }) {
  ...
  const addLineItem = (categoryId: string) => { setQuotes((prev) => prev.map((q) => { if (q.id !== selectedQuoteId) return q; return { ...q, categories: q.categories.map((cat) => { if (cat.id !== categoryId) return cat; const newItem: QuoteLineItem = { id: crypto.randomUUID?.() || `li${Date.now()}`, itemNumber: String(cat.lineItems.length + 1).padStart(2, "0"), description: "", unitCost: 0, quantity: 0, markupPercent: 0.35, sellPrice: 0, costTotal: 0, sellTotal: 0, profit: 0, jmdConversion: 0 }; return { ...cat, lineItems: [...cat.lineItems, newItem] }; }) }; })); };

  const handleCreateWorkbook = async () => {
    if (!selectedProject) { setShowProjectSelect(true); return; }
    const cats: QuoteCategory[] = [
      { id: crypto.randomUUID?.()||"syn1", name: "Video Security Equipment", type: "Video Surveillance", lineItems: [] },
      { id: crypto.randomUUID?.()||"syn2", name: "Access Control Equipment", type: "Access Control", lineItems: [] },
      { id: crypto.randomUUID?.()||"syn3", name: "Professional Services", type: "Both", lineItems: [] },
      { id: crypto.randomUUID?.()||"syn4", name: "Software", type: "Both", lineItems: [] },
      { id: crypto.randomUUID?.()||"syn5", name: "Compute & Storage", type: "Both", lineItems: [] },
      { id: crypto.randomUUID?.()||"syn6", name: "Networking", type: "Both", lineItems: [] },
    ];
    const newQuote: Quote = { id: crypto.randomUUID?.()||`q${Date.now()}`, clientName: selectedProject.client, refNumber: `Q-${new Date().getFullYear()}-${String(quotes.length+1).padStart(3,"0")}`, date: new Date().toISOString().slice(0,10), status: "draft", quoteType: "Both", categories: cats, exchangeRate, projectId: selectedProject.id };
    setQuotes((prev) => [...prev, newQuote]); setSelectedQuoteId(newQuote.id);
    try { await API.quotes.create(newQuote); toast.success("Workbook created"); } catch { toast.error("Failed to save"); }
  };

  const handleExportPDF = async () => { setGeneratingPDF(true); try { const { jsPDF } = await import("jspdf"); const doc = new jsPDF(); doc.setFontSize(18); doc.text("Workbook Synthesis", 14, 22); doc.setFontSize(11); let y = 35;
    doc.text(`Client: ${selectedQuote?.clientName||""}`,14,y); y+=7; doc.text(`Ref: ${selectedQuote?.refNumber||""}`,14,y); y+=12;
    quoteCategories.forEach((cat) => { doc.setFontSize(13); doc.text(cat.name,14,y); y+=8; doc.setFontSize(10);
      cat.lineItems.filter(li=>li.quantity>0).forEach((li) => { const recalc=recalcLineItem(li); doc.text(`${li.description} x${li.quantity} @ $${li.unitCost.toFixed(2)} = $${recalc.sellTotal.toFixed(2)}`,20,y); y+=6; if(y>270){doc.addPage();y=20;} });
      y+=4; });
    y+=4; doc.setFontSize(13); doc.text(`Subtotal: $${grandTotalPreTax.toFixed(2)}`,14,y); y+=7; doc.text(`GCT (15%): $${gctAmount.toFixed(2)}`,14,y); y+=7; doc.setFontSize(15); doc.text(`Grand Total: $${grandTotal.toFixed(2)}`,14,y);
    doc.save(`Workbook_${selectedQuote?.refNumber||"draft"}.pdf`); toast.success("PDF exported");
  } catch { toast.error("PDF generation failed"); } finally { setGeneratingPDF(false); } };

  const toggleCollapse = (id: string) => { setCollapsedCategories((prev) => { const next = new Set(prev); if (next.has(id)) next.delete(id); else next.add(id); return next; }); };
  ...
  const wbTabs = ["synthesis","software","compute-storage","networking","bom","workbook-pricing"];
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-56px)] overflow-hidden">
      ...
      <div className="px-4 md:px-6 py-4 md:py-5 flex flex-col md:flex-row md:items-center justify-between flex-shrink-0 gap-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
        ...
        <div className="flex items-center gap-2 flex-wrap"><div className="flex items-center gap-2 px-3 py-1 rounded-xl" style={G.subtle}><span className="text-[#484f58] text-[10px] font-bold uppercase">USD→JMD</span><input type="number" value={exchangeRate} onChange={(e) => setExchangeRate(parseFloat(e.target.value)||JMD_RATE)} className="w-20 h-7 rounded-lg bg-transparent text-white text-[12px] font-bold text-center focus:outline-none" style={G.input} /></div><button onClick={handleCreateWorkbook} className="flex items-center gap-1.5 h-8 px-3 md:px-4 rounded-xl text-white text-[11px] md:text-[12px] font-bold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]" style={{ background: "#3b82f6", boxShadow: "0 4px 16px rgba(59,130,246,0.35)" }}><Plus className="w-3.5 h-3.5" /> New Workbook</button>{selectedQuote && <button onClick={handleExportPDF} disabled={generatingPDF} className="flex items-center gap-1.5 h-8 px-3 rounded-xl text-[#8b949e] text-[10px] md:text-[11px] font-semibold hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform" style={G.btn}><FileText className="w-3.5 h-3.5" /> Export PDF</button>}</div>
      </div>
      {!selectedQuote ? <div className="flex-1 flex items-center justify-center"><EmptyState icon={DollarSign} title="No workbook selected" description="Create a new workbook to get started." action={{ label: "New Workbook", onClick: handleCreateWorkbook }} /></div> : (
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="flex items-center gap-1 px-4 md:px-6 py-2 overflow-x-auto flex-shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)", scrollbarWidth: "none" }}>
            {wbTabs.map((tab) => (<button key={tab} onClick={() => setActiveTab(tab)} className={clsx("h-8 px-3 rounded-lg text-[11px] md:text-[12px] font-semibold whitespace-nowrap transition-all cursor-pointer active:scale-[0.97] transition-transform", activeTab === tab ? "text-white" : "text-[#8b949e] hover:text-white")} style={activeTab === tab ? { background: "rgba(59,130,246,0.20)", border: "1px solid rgba(59,130,246,0.30)" } : undefined}>{tab.split("-").map(w => w.charAt(0).toUpperCase()+w.slice(1)).join(" ")}</button>))}
          </div>
          <div className="flex-1 overflow-y-auto px-3 md:px-5 py-4 md:py-5 space-y-4" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.08) transparent" }}>
            {activeTab === "synthesis" && quoteCategories.map((category) => { const activeItems = category.lineItems.filter((li) => li.quantity > 0); const catSubtotal = activeItems.reduce((s, li) => s + li.sellTotal, 0); const isCollapsed = collapsedCategories.has(category.id); return (
              <div key={category.id} className="rounded-2xl overflow-hidden" style={G.card}>
                <button onClick={() => toggleCollapse(category.id)} className="w-full px-4 md:px-5 py-3 flex items-center justify-between hover:bg-white/[0.02] transition-colors cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]" style={{ borderBottom: isCollapsed ? "none" : "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{ background: category.type === "Video Surveillance" ? "#3b82f6" : category.type === "Access Control" ? "#10b981" : "#8b5cf6" }} /><h3 className="text-white text-[13px] md:text-[14px] font-bold">{category.name}</h3><span className="text-[#484f58] text-[10px] font-semibold">({activeItems.length})</span></div><div className="flex items-center gap-3"><span className="text-[#8b949e] text-[11px] md:text-[12px] font-bold">{fmt(catSubtotal)}</span>{isCollapsed ? <ChevronDown className="w-4 h-4 text-[#484f58]" /> : <ChevronUp className="w-4 h-4 text-[#484f58]" />}</div></button>
                {!isCollapsed && (category.lineItems.length === 0 ? <div className="px-4 py-8 text-center"><p className="text-[#484f58] text-[12px]">No line items. <button onClick={() => addLineItem(category.id)} className="text-blue-400 hover:underline">Add one</button></p></div> : (
                  <div className="overflow-x-auto"><table className="w-full" style={{ minWidth: "700px" }}><thead><tr style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>{["#","Description","Unit Cost","Qty","Markup %","Sell Price","Total","Profit"].map((h) => (<th key={h} className="px-3 md:px-4 py-2.5 text-[#484f58] text-[9px] md:text-[10px] font-bold uppercase tracking-widest text-left">{h}</th>))}</tr></thead><tbody>{category.lineItems.map((item, idx) => { const recalc = recalcLineItem(item); return (<tr key={item.id} className="hover:bg-white/[0.02] transition-colors" style={{ borderBottom: "1px solid rgba(255,255,255,0.03)" }}><td className="px-3 md:px-4 py-2.5 text-[#484f58] text-[10px] font-mono">{String(idx+1).padStart(2,"0")}</td><td className="px-3 md:px-4 py-2.5"><input value={item.description} onChange={(e) => updateQuoteLineItem(category.id, item.id, { description: e.target.value })} className="bg-transparent text-white text-[11px] font-semibold w-full min-w-[120px] focus:outline-none" /></td><td className="px-3 md:px-4 py-2.5"><input type="number" value={item.unitCost} onChange={(e) => updateQuoteLineItem(category.id, item.id, { unitCost: parseFloat(e.target.value)||0 })} className="bg-transparent text-white text-[11px] w-20 text-right focus:outline-none" style={G.input} /></td><td className="px-3 md:px-4 py-2.5"><input type="number" value={item.quantity} onChange={(e) => updateQuoteLineItem(category.id, item.id, { quantity: parseInt(e.target.value)||0 })} className="bg-transparent text-white text-[11px] w-16 text-center focus:outline-none" style={G.input} /></td><td className="px-3 md:px-4 py-2.5"><input type="number" value={Math.round(item.markupPercent*100)} onChange={(e) => updateQuoteLineItem(category.id, item.id, { markupPercent: (parseFloat(e.target.value)||0)/100 })} className="bg-transparent text-white text-[11px] w-16 text-center focus:outline-none" style={G.input} /><span className="text-[#484f58] text-[10px]">%</span></td><td className="px-3 md:px-4 py-2.5 text-white text-[11px] font-bold tabular-nums text-right">{fmt(recalc.sellPrice)}</td><td className="px-3 md:px-4 py-2.5 text-white text-[11px] font-bold tabular-nums text-right">{fmt(recalc.sellTotal)}</td><td className="px-3 md:px-4 py-2.5 text-[10px] font-bold tabular-nums text-right" style={{ color: recalc.profit>=0?"#34d399":"#f87171" }}>{fmt(recalc.profit)}</td></tr>); })}</tbody></table></div>
                ))}
              </div>
            ); })}
            {activeTab === "synthesis" && quoteCategories.length > 0 && (
              <div className="rounded-2xl p-4 md:p-5" style={{ ...G.card, background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.20)" }}>
                ...
                <div className="space-y-2">{categorySubtotals.map((cs) => (<div key={cs.category.id} className="flex items-center justify-between py-1.5" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}><div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{ background: cs.category.type==="Video Surveillance"?"#3b82f6":cs.category.type==="Access Control"?"#10b981":"#8b5cf6" }} /><span className="text-[#8b949e] text-[12px] font-semibold">{cs.category.name}</span></div><span className="text-white text-[13px] font-bold tabular-nums">{fmt(cs.subtotal)}</span></div>))}
                  <div className="flex items-center justify-between py-2"><span className="text-[#8b949e] text-[12px] font-semibold">Subtotal</span><span className="text-white text-[14px] font-bold">{fmt(grandTotalPreTax)}</span></div>
                  <div className="flex items-center justify-between py-2"><span className="text-[#8b949e] text-[12px] font-semibold">GCT (15%)</span><span className="text-[#8b949e] text-[13px] font-bold">{fmt(gctAmount)}</span></div>
                  <div className="flex items-center justify-between py-2 pt-3" style={{ borderTop: "2px solid rgba(255,255,255,0.12)" }}><span className="text-white text-[14px] font-bold">Grand Total</span><span className="text-white text-[1.1rem] font-extrabold" style={{ color: "#60a5fa" }}>{fmt(grandTotal)}</span></div>
                </div>
              </div>
            )}
            {activeTab !== "synthesis" && <EmptyState icon={Package} title={`${activeTab.split("-").map(w=>w.charAt(0).toUpperCase()+w.slice(1)).join(" ")} tab`} description="This tab is ready for content." />}
          </div>
        </div>
      )}
    </div>
...
function DeviceSpecModal({ device, onClose }: { device: CatalogDevice; onClose: () => void }) {
  ...
  const specs: { label: string; value?: string }[] = [{ label: "SKU", value: device.sku },{ label: "Category", value: cc.label },{ label: "Resolution", value: device.resolution },{ label: "Sensor", value: device.sensor },{ label: "Lens", value: device.lens },{ label: "Frame Rate", value: device.frameRate },{ label: "FOV", value: device.fov },{ label: "Night Vision", value: device.nightVision },{ label: "Weather", value: device.weatherRating },{ label: "Power", value: device.powerInput },{ label: "Operating Temp", value: device.operatingTemp }].filter((s) => !!s.value);
  return (
...
function DeviceStore({ navigate: _navigate }: { navigate: (p: Page) => void }) {
  ...
  const handleCSVUpload = async (e: React.ChangeEvent<HTMLInputElement>) => { const file = e.target.files?.[0]; if (!file) return; setCsvUploading(true); try { const text = await file.text(); const lines = text.split("\n").filter(l => l.trim()); const headers = lines[0].split(",").map(h => h.trim().replace(/"/g,"")); const parsed = lines.slice(1).map(line => { const vals = line.split(",").map(v => v.trim().replace(/"/g,"")); const obj: any = {}; headers.forEach((h,i) => { obj[h] = vals[i]; }); return { model: obj.Model||obj.model||"", manufacturer: obj.Manufacturer||obj.manufacturer||"Unknown", category: "camera" as const, resolution: obj["Max Video Resolution"]||undefined, lens: obj["Sensor/Lens/Horizontal FOV"]||undefined, price: parseFloat(obj.Price||"0")||undefined, sku: obj.SKU||obj.Model||undefined, imageUrl: obj["Image URL"]||undefined, tags: [] as DeviceTag[] }; }); if (parsed.length > 0) { await API.devices.bulk(parsed); toast.success(`Imported ${parsed.length} devices`); fetchDevices(); } } catch { toast.error("CSV import failed"); } finally { setCsvUploading(false); e.target.value = ""; } };
  ...
  return (
    <div className="px-3 md:px-5 py-4 md:py-6">
      ...
      <div className="flex items-center justify-between mb-4 md:mb-6"><div><h1 className="text-white font-bold text-lg md:text-xl tracking-tight">Device Store</h1><p className="text-[#8b949e] text-[11px] md:text-[13px] mt-0.5">{filtered.length} products</p></div><div className="flex items-center gap-2"><label className="flex items-center gap-1.5 h-8 px-3 rounded-xl text-[#8b949e] text-[11px] font-semibold hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform" style={G.btn}><Upload className="w-3.5 h-3.5" /> {csvUploading ? "Importing…" : "Import CSV"}<input type="file" accept=".csv" className="hidden" onChange={handleCSVUpload} disabled={csvUploading} /></label><div className="flex items-center h-8 rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.10)" }}><button onClick={() => setViewMode("grid")} className="h-full px-2.5 flex items-center justify-center transition-all cursor-pointer active:scale-[0.97] transition-transform" style={viewMode==="grid"?{background:"#3b82f620",color:"#60a5fa"}:{color:"#484f58"}}><Grid3x3 className="w-3.5 h-3.5" /></button><button onClick={() => setViewMode("table")} className="h-full px-2.5 flex items-center justify-center transition-all cursor-pointer active:scale-[0.97] transition-transform" style={viewMode==="table"?{background:"#3b82f620",color:"#60a5fa"}:{color:"#484f58"}}><List className="w-3.5 h-3.5" /></button></div></div></div>
      <div className="mb-4 md:mb-5 flex items-center gap-2 md:gap-3 flex-wrap"><div className="relative flex-1 min-w-[160px] max-w-sm"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#484f58]" /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search…" className="h-9 rounded-xl pl-9 w-full text-[12px] text-[#e6edf3] placeholder:text-[#484f58] focus:outline-none" style={G.input} /></div><div className="flex gap-1.5 md:gap-2 flex-wrap">{categories.map((c) => (<button key={c.id} onClick={() => setCategoryFilter(c.id)} className="h-8 md:h-9 px-2.5 md:px-3.5 rounded-xl text-[11px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform whitespace-nowrap" style={categoryFilter===c.id?{background:"rgba(59,130,246,0.15)",border:"1px solid rgba(59,130,246,0.35)",color:"#60a5fa"}:{...G.btn,color:"#8b949e"}}>{c.label}</button>))}</div></div>
      {devices.length === 0 ? <EmptyState icon={Store} title="Device store is empty" description="Import a CSV to populate the catalog." /> : filtered.length === 0 ? <EmptyState icon={Search} title="No devices match" description="Try adjusting your search." /> : viewMode === "grid" ? (
        <div className="grid gap-3 md:gap-4" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))" }}>{filtered.map((device) => { const cc = CAT_COLOR[device.category] ?? CAT_COLOR.other; return (<div key={device.id} onClick={() => setSelectedDevice(device)} className="rounded-2xl overflow-hidden cursor-pointer group transition-all md:hover:-translate-y-1" style={{ ...G.card }}><div className="relative h-32 md:h-36 overflow-hidden" style={{ background: "rgba(255,255,255,0.03)" }}>{device.imageUrl ? <img src={device.imageUrl} alt={device.model} className="w-full h-full object-contain p-3 opacity-70 group-hover:opacity-90 group-hover:scale-105 transition-all duration-500" /> : <div className="w-full h-full flex items-center justify-center"><Camera className="w-12 h-12 text-[#484f58]" /></div>}<div className="absolute top-2.5 left-2.5"><span className="inline-block px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase" style={{ background: cc.bg, color: cc.text }}>{cc.label}</span></div></div><div className="p-3 md:p-3.5"><p className="text-[#8b949e] text-[9px] font-semibold">{device.manufacturer}</p><p className="text-white text-[12px] font-bold mt-0.5 truncate">{device.model}</p>{device.resolution && <p className="text-[#484f58] text-[9px] mt-1 truncate">{device.resolution}</p>}<div className="flex items-center justify-between mt-2 pt-2" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}><span className="text-[#484f58] text-[8px] font-mono">{device.sku}</span><span className="font-bold text-[11px]" style={{ color: cc.text }}>{device.price ? fmt(device.price) : "—"}</span></div></div></div>); })}</div>
      ) : (
        <div className="rounded-2xl overflow-hidden" style={G.card}><div className="overflow-x-auto"><table className="w-full" style={{ minWidth: "700px" }}><thead><tr style={{ background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>{["","Model","Manufacturer","Category","Resolution","SKU","Price"].map((h) => (<th key={h} className={`${h==="Price"?"text-right":"text-left"} px-3 md:px-4 py-3 text-[#484f58] text-[10px] font-bold uppercase tracking-widest`}>{h}</th>))}</tr></thead><tbody>{filtered.map((device, i) => { const cc = CAT_COLOR[device.category] ?? CAT_COLOR.other; return (<tr key={device.id} onClick={() => setSelectedDevice(device)} className="cursor-pointer hover:bg-white/[0.02] transition-colors group" style={{ borderBottom: i<filtered.length-1?"1px solid rgba(255,255,255,0.04)":"none" }}><td className="px-3 md:px-4 py-2.5"><div className="w-9 h-9 rounded-xl overflow-hidden flex-shrink-0" style={{ background: "rgba(255,255,255,0.04)" }}>{device.imageUrl ? <img src={device.imageUrl} alt={device.model} className="w-full h-full object-contain p-1 opacity-70 group-hover:opacity-100" /> : <Camera className="w-4 h-4 text-[#484f58] m-auto mt-2.5" />}</div></td><td className="px-3 md:px-4 py-2.5 text-white text-[11px] font-semibold">{device.model}</td><td className="px-3 md:px-4 py-2.5 text-[#8b949e] text-[10px]">{device.manufacturer}</td><td className="px-3 md:px-4 py-2.5"><span className="inline-block px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase" style={{ background: cc.bg, color: cc.text }}>{cc.label}</span></td><td className="px-3 md:px-4 py-2.5 text-[#8b949e] text-[10px]">{device.resolution||"—"}</td><td className="px-3 md:px-4 py-2.5 text-[#484f58] text-[10px] font-mono">{device.sku}</td><td className="px-3 md:px-4 py-2.5 text-right"><span className="text-white text-[11px] font-bold">{device.price?fmt(device.price):"—"}</span></td></tr>); })}</tbody></table></div></div>
      )}
    </div>
  );
}
...
function InstallTracker({ navigate: _navigate }: { navigate: (p: Page) => void }) {
  ...
  const handleAddZone = async (e: React.FormEvent) => { e.preventDefault(); if (!newZoneName.trim()) return; try { const result = await API.install.createZone({ name: newZoneName.trim() }); setZones((prev) => [...prev, result]); setNewZoneName(""); setShowAddZone(false); toast.success("Zone created"); } catch { toast.error("Failed to create zone"); } };
  const handleAddDevice = async (e: React.FormEvent) => { e.preventDefault(); if (!newDeviceName.trim() || !newDeviceZone) return; try { const result = await API.install.addDevice(newDeviceZone, { name: newDeviceName.trim(), type: newDeviceType, location: newDeviceLocation.trim() || "TBD", status: "pending", assignee: newDeviceAssignee || "Unassigned" }); setZones((prev) => prev.map((z) => z.id === newDeviceZone ? { ...z, devices: [...z.devices, result] } : z)); setShowAddDevice(false); setNewDeviceName(""); setNewDeviceLocation(""); setExpandedZone(newDeviceZone); toast.success("Device added"); } catch { toast.error("Failed to add device"); } };
  ...
  return (
    <div className="px-3 md:px-5 py-4 md:py-6 max-w-[1100px]">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 md:mb-6 gap-3"><div><h1 className="text-white font-bold text-lg md:text-xl tracking-tight">Install Tracker</h1><p className="text-[#8b949e] text-[11px] md:text-[13px] mt-0.5">{total} devices across {zones.length} zones</p></div><div className="flex items-center gap-2"><button onClick={() => setShowAddZone(true)} className="flex items-center gap-1.5 h-8 px-3 rounded-xl text-[#8b949e] text-[11px] font-semibold hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform" style={G.btn}><Plus className="w-3.5 h-3.5" /> Add Zone</button><button onClick={() => { setShowAddDevice(true); if (zones.length > 0 && !newDeviceZone) setNewDeviceZone(zones[0].id); }} className="flex items-center gap-1.5 h-8 px-3 md:px-4 rounded-xl text-white text-[11px] font-bold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]" style={{ background: "#3b82f6", boxShadow: "0 4px 16px rgba(59,130,246,0.35)" }}><Plus className="w-3.5 h-3.5" /> Add Device</button></div></div>

      {showAddZone && (<div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={() => setShowAddZone(false)}><div className="absolute inset-0" style={{ background: "rgba(0,0,0,0.65)", backdropFilter: "blur(8px)" }} /><motion.div initial={{ opacity: 0, scale: 0.94, y: 14 }} animate={{ opacity: 1, scale: 1, y: 0 }} onClick={(e) => e.stopPropagation()} className="relative z-10 w-full max-w-[400px] rounded-3xl" style={{ background: "rgba(7,12,26,0.92)", backdropFilter: "blur(52px)", border: "1px solid rgba(255,255,255,0.13)", boxShadow: "0 32px 80px rgba(0,0,0,0.9)" }}><form onSubmit={handleAddZone} className="p-6 space-y-4"><h2 className="text-white text-[1rem] font-bold">New Zone</h2><input value={newZoneName} onChange={(e) => setNewZoneName(e.target.value)} placeholder="Zone name" className="w-full h-9 rounded-xl px-3 text-[#e6edf3] text-[12px] focus:outline-none" style={G.input} /><div className="flex gap-3"><button type="button" onClick={() => setShowAddZone(false)} className="flex-1 h-10 rounded-xl text-[#8b949e] text-[13px] font-semibold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]" style={G.btn}>Cancel</button><button type="submit" className="flex-1 h-10 rounded-xl text-white text-[13px] font-bold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]" style={{ background: "#3b82f6" }}>Create</button></div></form></motion.div></div>)}

      {showAddDevice && (<div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={() => setShowAddDevice(false)}><div className="absolute inset-0" style={{ background: "rgba(0,0,0,0.65)", backdropFilter: "blur(8px)" }} /><motion.div initial={{ opacity: 0, scale: 0.94, y: 14 }} animate={{ opacity: 1, scale: 1, y: 0 }} onClick={(e) => e.stopPropagation()} className="relative z-10 w-full max-w-[440px] max-h-[90vh] overflow-y-auto rounded-3xl" style={{ background: "rgba(7,12,26,0.92)", backdropFilter: "blur(52px)", border: "1px solid rgba(255,255,255,0.13)" }}><div className="flex items-center justify-between px-5 md:px-6 pt-5 pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}><h2 className="text-white text-[1rem] font-bold">Add Device</h2><button onClick={() => setShowAddDevice(false)} className="w-7 h-7 rounded-xl flex items-center justify-center hover:bg-white/[0.08] cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px]" style={{ border: "1px solid rgba(255,255,255,0.10)" }}><X className="w-4 h-4 text-[#8b949e]" /></button></div><form onSubmit={handleAddDevice} className="px-5 md:px-6 py-5 space-y-4"><input value={newDeviceName} onChange={(e) => setNewDeviceName(e.target.value)} placeholder="Device name" className="w-full h-9 rounded-xl px-3 text-[#e6edf3] text-[12px] focus:outline-none" style={G.input} /><div className="grid grid-cols-2 gap-3"><select value={newDeviceType} onChange={(e) => setNewDeviceType(e.target.value as InstallDevice["type"])} className="w-full h-9 rounded-xl px-3 text-[#e6edf3] text-[12px] focus:outline-none appearance-none cursor-pointer" style={G.input}>{["camera","access","nvr","door","panel","power","server"].map((t) => (<option key={t} value={t} style={{ background: "#0d1117" }}>{t.charAt(0).toUpperCase()+t.slice(1)}</option>))}</select><select value={newDeviceZone} onChange={(e) => setNewDeviceZone(e.target.value)} className="w-full h-9 rounded-xl px-3 text-[#e6edf3] text-[12px] focus:outline-none appearance-none cursor-pointer" style={G.input}>{zones.map((z) => (<option key={z.id} value={z.id} style={{ background: "#0d1117" }}>{z.name}</option>))}</select></div><input value={newDeviceLocation} onChange={(e) => setNewDeviceLocation(e.target.value)} placeholder="Location" className="w-full h-9 rounded-xl px-3 text-[#e6edf3] text-[12px] focus:outline-none" style={G.input} /><input value={newDeviceAssignee} onChange={(e) => setNewDeviceAssignee(e.target.value)} placeholder="Technician" className="w-full h-9 rounded-xl px-3 text-[#e6edf3] text-[12px] focus:outline-none" style={G.input} /><div className="flex gap-3 pt-1"><button type="button" onClick={() => setShowAddDevice(false)} className="flex-1 h-10 rounded-xl text-[#8b949e] text-[13px] font-semibold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]" style={G.btn}>Cancel</button><button type="submit" disabled={!newDeviceName.trim() || !newDeviceZone} className="flex-1 h-10 rounded-xl text-white text-[13px] font-bold disabled:opacity-40 cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]" style={{ background: "#3b82f6" }}>Add Device</button></div></form></motion.div></div>)}

      {zones.length === 0 ? <EmptyState icon={CheckSquare} title="No install zones" description="Create your first zone to start tracking." action={{ label: "Add Zone", onClick: () => setShowAddZone(true) }} /> : (
        <>
          <div className="rounded-2xl p-4 md:p-5 mb-4 md:mb-5" style={G.card}><div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-3"><div><p className="text-white text-[1.6rem] md:text-[2rem] font-bold">{pct}%</p></div><div className="grid grid-cols-4 gap-3">{[{ label: "Complete", count: complete, color: "text-emerald-400" },{ label: "In Progress", count: inProg, color: "text-blue-400" },{ label: "Failed", count: failed, color: "text-rose-400" },{ label: "Pending", count: total-complete-inProg-failed, color: "text-[#484f58]" }].map((s) => (<div key={s.label}><p className={clsx("text-[1.1rem] md:text-[1.3rem] font-bold", s.color)}>{s.count}</p><p className="text-[#484f58] text-[9px]">{s.label}</p></div>))}</div></div><div className="relative w-full h-3 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}><div className="absolute left-0 top-0 h-full bg-gradient-to-r from-blue-600 to-emerald-500 rounded-full" style={{ width: `${pct}%` }} /></div></div>
          <div className="space-y-2 md:space-y-3">{zones.map((zone) => { const zComplete = zone.devices.filter((d) => d.status === "complete").length; const zPct = zone.devices.length > 0 ? Math.round((zComplete / zone.devices.length) * 100) : 0; const isExpanded = expandedZone === zone.id; return (<div key={zone.id} className="rounded-2xl overflow-hidden" style={G.card}><button onClick={() => setExpandedZone(isExpanded ? null : zone.id)} className="w-full flex items-center gap-3 md:gap-4 px-4 md:px-5 py-3 md:py-4 hover:bg-white/[0.02] transition-colors cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"><div className="flex-1 flex items-center gap-3 min-w-0"><div className={clsx("w-7 h-7 md:w-8 md:h-8 rounded-xl flex items-center justify-center flex-shrink-0", zPct===100?"bg-emerald-500/15":zPct>0?"bg-blue-500/15":"bg-white/[0.04]")}>{zPct===100?<CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />:zPct>0?<Clock className="w-3.5 h-3.5 text-blue-400" />:<AlertCircle className="w-3.5 h-3.5 text-[#484f58]" />}</div><div className="min-w-0"><p className="text-white text-[12px] md:text-[13px] font-bold text-left">{zone.name}</p><p className="text-[#484f58] text-[10px]">{zone.devices.length} devices · {zComplete} complete</p></div></div><div className="flex items-center gap-3 flex-shrink-0"><div className="w-20 md:w-24 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)" }}><div className={clsx("h-full rounded-full", zPct===100?"bg-emerald-500":"bg-blue-500")} style={{ width: `${zPct}%` }} /></div><span className={clsx("text-[11px] font-bold w-10 text-right", zPct===100?"text-emerald-400":"text-white")}>{zPct}%</span>{isExpanded?<ChevronUp className="w-4 h-4 text-[#484f58]" />:<ChevronDown className="w-4 h-4 text-[#484f58]" />}</div></button>{isExpanded && (<div style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>{zone.devices.map((device, i) => { const meta = STATUS_META[device.status]; const TypeIcon = typeIcons[device.type] ?? Camera; return (<div key={device.id} className={clsx("grid gap-2 md:gap-4 px-3 md:px-5 py-3 items-center hover:bg-white/[0.015] transition-colors", i%2===1&&"bg-white/[0.01]")} style={{ gridTemplateColumns: "36px 2fr 1.5fr 120px 1fr", borderTop: "1px solid rgba(255,255,255,0.04)" }}><div className="w-7 h-7 rounded-xl flex items-center justify-center" style={{ background: "rgba(255,255,255,0.04)" }}><TypeIcon className="w-3.5 h-3.5 text-[#484f58]" /></div><div className="min-w-0"><p className="text-white text-[11px] font-semibold truncate">{device.name}</p></div><p className="text-[#8b949e] text-[10px] truncate">{device.location}</p><span className="text-[#8b949e] text-[10px] truncate">{device.assignee}</span><select value={device.status} onChange={(e) => updateStatus(zone.id, device.id, e.target.value as InstallStatus)} className={clsx("w-full h-7 rounded-xl border px-2 text-[10px] font-bold appearance-none cursor-pointer", meta.bg, meta.color)} style={{ backgroundColor: "transparent" }}>{Object.entries(STATUS_META).map(([val, m]) => (<option key={val} value={val} style={{ background: "#0d1117", color: "#e6edf3" }}>{m.label}</option>))}</select></div>); })}</div>)}</div>); })}</div>
        </>
      )}
    </div>
  );
