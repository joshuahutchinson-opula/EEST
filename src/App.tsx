...
import {
  ...
  DoorOpen, PanelRight, Zap, Server, Cable, Box,
} from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { id: "workbook" as Page, label: "Workbook" },
...
const CANVAS_DEVICE_LIBRARY = [
  { category: "Fixed Cameras", color: "#3b82f6", items: ["Axis P3245-V Dome", "Axis P3268-LVE 4K", "Axis P1448-LE 4K"] },
  { category: "PTZ Cameras", color: "#8b5cf6", items: ["Axis P5655-E PTZ", "Axis Q6075-E"] },
  { category: "Access Control", color: "#10b981", items: ["Suprema BioLite N2", "HID EVO Solo"] },
  { category: "Doors & Hardware", color: "#f59e0b", items: ["Electric Strike", "Maglock", "Door Contact"] },
  { category: "Panels & Power", color: "#f97316", items: ["PoE Switch 24-port", "Midspan Injector", "PDU"] },
  { category: "Servers & NVR", color: "#ec4899", items: ["Dell PowerEdge R450", "Axis S2212 NVR"] },
];
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "#070c1a" }}>
      <header className="h-12 flex items-center gap-2 md:gap-4 px-3 md:px-4 flex-shrink-0 z-40" style={G.liquidGlass}>
        <button onClick={() => navigate("project-detail")} className="flex items-center gap-1.5 text-[#8b949e] hover:text-white text-[11px] md:text-[12px] font-semibold transition-colors flex-shrink-0 cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"><ArrowLeft className="w-3.5 h-3.5" /> <span className="hidden md:inline">Back</span></button>
        <div className="w-px h-4 flex-shrink-0 hidden md:block" style={{ background: "rgba(255,255,255,0.10)" }} />
        <div className="flex items-center gap-2 flex-shrink-0"><div className="w-6 h-6 rounded-lg flex items-center justify-center" style={{ background: "rgba(139,92,246,0.2)", border: "1px solid rgba(139,92,246,0.3)" }}><Layers className="w-3.5 h-3.5 text-violet-400" /></div><span className="text-white text-[12px] font-bold leading-none hidden md:inline">Site Design</span></div>
        <div className="flex-1" />
        <button onClick={() => setView3D(!view3D)} className={clsx("flex items-center gap-1.5 h-7 px-2 md:px-3 rounded-xl text-[10px] md:text-[11px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform", view3D ? "text-violet-400" : "text-[#8b949e] hover:text-white")} style={view3D ? { background: "rgba(139,92,246,0.15)", border: "1px solid rgba(139,92,246,0.30)" } : G.btn}><Box className="w-3 h-3" /> {view3D ? "2D" : "3D"}</button>
        <button onClick={() => setShowFov(!showFov)} className={clsx("flex items-center gap-1.5 h-7 px-2 md:px-3 rounded-xl text-[10px] md:text-[11px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform", showFov ? "text-blue-400" : "text-[#8b949e] hover:text-white")} style={showFov ? { background: "rgba(59,130,246,0.15)", border: "1px solid rgba(59,130,246,0.30)" } : G.btn}><Eye className="w-3 h-3" /> <span className="hidden md:inline">FOV</span></button>
      ...
      <div className="flex-1 relative overflow-hidden">
        ...
        <div ref={canvasRef} className="absolute inset-0 flex items-center justify-center overflow-hidden" style={{ cursor: activeTool === "move" ? "grab" : activeTool === "select" ? "default" : "crosshair", touchAction: "none" }}
          ...
          <svg viewBox="0 0 990 610" className="w-full h-full max-w-none" style={{ transform: `scale(${canvasScale}) translate(${canvasOffset.x}px, ${canvasOffset.y}px)`, maxHeight: "calc(100vh - 140px)" }}>
            ...
            <rect x="640" y="50" width="270" height="150" fill="rgba(245,158,11,0.03)" stroke="rgba(255,255,255,0.08)" strokeWidth="1" /><text x="775" y="132" textAnchor="middle" fill="rgba(255,255,255,0.18)" fontSize="9" fontFamily="sans-serif">SERVER ROOM 1</text>
            <rect x="80" y="200" width="830" height="60" fill="rgba(255,255,255,0.01)" stroke="rgba(255,255,255,0.05)" strokeWidth="0.5" /><text x="495" y="234" textAnchor="middle" fill="rgba(255,255,255,0.08)" fontSize="8" fontFamily="sans-serif">MAIN CORRIDOR</text>
            ...
            {devices.map((dev) => {
              ...
              if (dev.type === "power") return (<g key={dev.id} onClick={() => setSelectedId(dev.id)} style={{ cursor: "pointer" }}><rect x={dev.x - 8} y={dev.y - 6} width="16" height="12" fill="rgba(239,68,68,0.15)" stroke={color} strokeWidth={isSelected ? 2 : 1} rx="1" /><text x={dev.x} y={dev.y + 1} textAnchor="middle" fill={color} fontSize="4" fontFamily="monospace">PWR</text></g>);
          ...
          {selectedQuote && quoteCategories.length > 0 && (
            <div className="rounded-2xl p-4 md:p-5" style={{ ...G.card, background: "rgba(59,130,246,0.06)", border: "1px solid rgba(59,130,246,0.20)" }}>
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-between py-1"><span className="text-[#484f58] text-[11px]">JMD Equivalent (Rate: {exchangeRate})</span><span className="text-[#8b949e] text-[12px] font-bold tabular-nums">J${(grandTotal * exchangeRate).toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span></div>
              </div>
            </div>
          )}
        </div>
