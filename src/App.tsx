...
import p1465leImg from "./assets/p1465le.png";
import p3245Img from "./assets/p3245.png";
import p5655eImg from "./assets/p5655e.png";
import q6075eImg from "./assets/q6075e.png";
import sarixImg from "./assets/Sarix-IBP-3Feature.png";
import opteraImg from "./assets/Optera270PageFeature.png";
...
type DeviceTag = "LPR" | "Night Vision" | "Thermal" | "PTZ" | "Panoramic" | "WDR" | "Lightfinder" | "IR";
...
const NAV_ITEMS = [
  ...
  { id: "device-store" as Page, label: "Device Store" },
];
...
function SearchPalette({ onClose, navigate }: { onClose: () => void; navigate: (p: Page) => void }) {
  ...
  const PAGE_LINKS: { label: string; sub: string; page: Page; icon: React.ElementType }[] = [
    ...
    { label: "Device Store", sub: "Product catalog & specifications", page: "device-store", icon: Store },
  ];
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-start justify-center pt-[12vh]" onClick={onClose}>
      ...
      <motion.div
        ...
        className="relative z-10 w-full max-w-[560px] rounded-2xl overflow-hidden max-md:mx-4"
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 h-14 flex items-center gap-3 md:gap-5 px-3 md:px-5"
      ...
      <div className="flex items-center gap-1 md:gap-1.5">
        ...
        <button onClick={() => setShowSearch(true)} className="flex items-center gap-2 h-8 px-2 md:px-3 rounded-xl text-[#8b949e] text-[12px] hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform"
          style={G.btn}>
          <Search className="w-3.5 h-3.5" />
          <span className="hidden md:inline">Search…</span>
          <kbd className="hidden md:inline ml-1 text-[10px] px-1.5 py-0.5 rounded font-mono opacity-60" style={{ background: "rgba(255,255,255,0.08)" }}>⌘K</kbd>
        </button>
        <div className="relative">
          <button onClick={() => { setShowNotifications(!showNotifications); setShowSettings(false); }}
            className="relative w-8 h-8 md:w-8 md:h-8 rounded-xl hover:bg-white/[0.06] flex items-center justify-center transition-colors cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0">
          ...
          {showNotifications && (
            <>
              ...
              <div className="absolute right-0 top-10 w-80 max-w-[90vw] z-[70] rounded-2xl overflow-hidden"
                ...
                {[
                  ...
                  <button key={i} onClick={() => setShowNotifications(false)}
                    className="w-full flex items-start gap-3 px-4 py-3 hover:bg-white/[0.04] transition-colors text-left cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
                ...
                <div className="px-4 py-2.5">
                  <button className="w-full text-center text-blue-400 text-[12px] font-semibold hover:text-blue-300 transition-colors cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]">
                    Mark all as read
                  </button>
                </div>
              </div>
        ...
        <div className="relative">
          <button onClick={() => { setShowSettings(!showSettings); setShowNotifications(false); }}
            className="w-8 h-8 md:w-8 md:h-8 rounded-xl hover:bg-white/[0.06] flex items-center justify-center transition-colors cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0">
          ...
          {showSettings && (
            <>
              ...
              <div className="absolute right-0 top-10 w-72 max-w-[90vw] z-[70] rounded-2xl overflow-hidden"
                style={{ background: "rgba(7,12,26,0.95)", backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)", border: "1px solid rgba(255,255,255,0.12)", boxShadow: "0 16px 48px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.09)" }}>
                ...
                <div className="p-3 space-y-1">
                  {[
                    ...
                    <div key={s.label} className="flex items-center justify-between px-3 py-2.5 rounded-xl hover:bg-white/[0.04] transition-colors min-h-[44px]">
                      <div>
                ...
                <div className="px-4 py-3" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
                  <button onClick={() => { setShowSettings(false); navigate("login"); }}
                    className="w-full flex items-center gap-2 text-rose-400 text-[12px] font-semibold hover:text-rose-300 transition-colors cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]">
        ...
        <button onClick={() => navigate("login")} className="flex items-center gap-2 h-8 pl-1.5 pr-2.5 rounded-xl hover:bg-white/[0.06] transition-colors ml-1 cursor-pointer active:scale-[0.97] transition-transform min-h-[44px] md:min-h-0">
          ...
          <ChevronDown className="w-3 h-3 text-[#484f58] hidden md:block" />
        </button>
      </div>
    </header>
  );
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <motion.div
        ...
        onClick={(e) => e.stopPropagation()}
        className="relative z-10 w-full max-w-[540px] max-h-[90vh] overflow-y-auto rounded-3xl overflow-hidden"
        ...
        <div className="px-5 md:px-7 pb-7 flex gap-2.5">
          ...
          <button onClick={() => { navigate("design-canvas"); onClose(); }}
            className="flex-1 h-10 rounded-xl flex items-center justify-center gap-2 text-[#e6edf3] text-[13px] font-bold transition-all duration-150 hover:bg-white/[0.12] cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
            style={G.btn}>
...
function KanbanCard({ project, column, dragging, onDragStart, onDragEnd, onClick, onDelete }: {
  ...
  return (
    <div
      ...
      <div className="absolute inset-0 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none hidden md:block"
        style={{ boxShadow: "0 8px 32px rgba(0,0,0,0.7), inset 0 1px 0 rgba(255,255,255,0.14)" }} />
      <div className="absolute left-2 top-4 opacity-0 group-hover:opacity-20 transition-opacity pointer-events-none hidden md:block">
      ...
      <div className="p-3 md:p-4 md:pl-5">
        <div className="flex items-start justify-between gap-2 mb-2">
          ...
          <div className="relative flex-shrink-0">
            <button onClick={(e) => { e.stopPropagation(); setMenuOpen(!menuOpen); }}
              className="opacity-0 group-hover:opacity-100 transition-opacity w-6 h-6 rounded-lg hover:bg-white/10 flex items-center justify-center mt-0.5 cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0">
        ...
        <div className="flex items-center gap-1.5 mb-1">
          ...
          <span className="text-[#8b949e] text-[10px] md:text-[11px] font-semibold truncate">{project.client}</span>
        </div>
        <div className="flex items-center gap-1.5 mb-3">
          <MapPin className="w-3 h-3 text-[#484f58] flex-shrink-0" />
          <span className="text-[#484f58] text-[10px] md:text-[11px] truncate">{project.location}</span>
        </div>
...
function KanbanColumn({ column, projects, totalValue, dragging, isOver, onDragStart, onDragEnd, onDragOver, onDragLeave, onDrop, onCardClick, onDelete }: {
  ...
  return (
    <div onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}
      ...
      <div className="px-3.5 pt-3.5 pb-3 flex-shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-1 flex-shrink-0 ml-2">
            <span className="text-[#8b949e] text-[10px] md:text-[11px] px-1.5 py-0.5 rounded-full font-bold"
      ...
      <div className="flex-1 p-2 space-y-2 overflow-y-auto" style={{ scrollbarWidth: "none", scrollBehavior: "smooth", WebkitOverflowScrolling: "touch", minHeight: "120px" }}>
        {projects.map((p) => (
...
function NewProjectModal({ onClose, onAdd }: { onClose: () => void; onAdd: (p: Project) => void }) {
  ...
  const inputCls = "w-full h-9 md:h-9 rounded-xl px-3 text-[#e6edf3] text-[12px] placeholder:text-[#484f58] focus:outline-none focus:ring-1 focus:ring-blue-500/50 transition-all";
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <motion.div
        ...
        transition={{ type: "spring", damping: 26, stiffness: 360 }}
        onClick={(e) => e.stopPropagation()}
        className="relative z-10 w-full max-w-[600px] max-h-[90vh] overflow-y-auto rounded-3xl overflow-hidden"
        ...
        <div className="relative flex items-center justify-between px-5 md:px-7 pt-5 md:pt-7 pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          <div>
            <h2 className="text-white text-[1rem] md:text-[1.1rem] font-bold tracking-tight">New Project</h2>
            <p className="text-[#8b949e] text-[12px] mt-0.5">Add a new project to the pipeline</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-white/[0.08] transition-colors cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px]"
            style={{ border: "1px solid rgba(255,255,255,0.10)" }}>
            <X className="w-4 h-4 text-[#8b949e]" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="px-5 md:px-7 py-5 space-y-4" style={{ scrollbarWidth: "none" }}>
          ...
          <div className="px-5 md:px-7 pb-7 pt-4 flex gap-3" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
            <button type="button" onClick={onClose}
              className="flex-1 h-10 rounded-xl text-[#8b949e] text-[13px] font-semibold hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
              style={G.btn}>
              Cancel
            </button>
            <button type="submit" disabled={!canSubmit}
              className="flex-1 h-10 rounded-xl text-white text-[13px] font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <motion.div
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-white/[0.08] cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px]"
            style={{ border: "1px solid rgba(255,255,255,0.10)" }}>
            <X className="w-4 h-4 text-[#8b949e]" />
          </button>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="flex gap-3 mt-5">
            ...
            <button onClick={handleUpload} disabled={!file}
              className="flex-1 h-10 rounded-xl text-white text-[13px] font-bold transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center gap-2 cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <motion.div
        ...
        transition={{ type: "spring", damping: 26, stiffness: 360 }}
        onClick={(e) => e.stopPropagation()}
        className="relative z-10 w-full max-w-[500px] max-h-[80vh] overflow-y-auto rounded-3xl overflow-hidden"
        ...
        <div className="max-h-[340px] overflow-y-auto" style={{ scrollbarWidth: "none" }}>
          {filtered.map((p) => (
            <button
              ...
              className="w-full flex items-center gap-3 px-5 py-3 hover:bg-white/[0.04] transition-colors text-left cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
    ...
    <div className="flex flex-col h-[calc(100vh-56px)]">
      ...
      <div className="px-3 md:px-5 pt-4 md:pt-6 pb-4 md:pb-5 flex-shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="flex items-center justify-between mb-4 md:mb-5">
          <div>
            <h1 className="text-white font-bold text-lg md:text-xl tracking-tight">Project Pipeline</h1>
            <p className="text-[#8b949e] text-[11px] md:text-[13px] mt-0.5">{projects.length} projects · FY 2026 · Q3</p>
          </div>
          <div className="flex items-center gap-2">
          ...
          {[
            ...
            <div key={stat.label} className="rounded-2xl p-3 md:p-4 transition-all duration-200 md:hover:-translate-y-0.5" style={G.card}>
              <div className="flex items-center justify-between mb-2 md:mb-3">
                ...
                <div className="w-7 h-7 md:w-8 md:h-8 rounded-xl flex items-center justify-center"
                  style={{ background: `${STAT_COLORS[i]}18`, border: `1px solid ${STAT_COLORS[i]}30` }}>
                  <stat.icon className="w-3 h-3 md:w-3.5 md:h-3.5" style={{ color: STAT_COLORS[i] }} />
                </div>
              </div>
              <p className="text-white text-[1.4rem] md:text-[2rem] font-extrabold tracking-tight leading-none mb-1 md:mb-1.5">
                {stat.isPct ? `${stat.value}%` : fmt(stat.value as number, stat.compact)}
              </p>
              <div className="flex items-center justify-between">
                <p className="text-[#484f58] text-[10px] md:text-[11px]">
                  {stat.isPct ? `${won.length} of ${closed.length} closed` : stat.label === "Active Pipeline" ? `${active.length} active` : stat.label === "In Negotiation" ? `${negotiation.length} projects` : "Active projects"}
                </p>
                <p className={clsx("text-[10px] md:text-[11px] font-bold", stat.pos === true && "text-emerald-400", stat.pos === false && "text-rose-400", stat.pos === null && "text-amber-400")}>{stat.change}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-x-auto px-3 md:px-5 py-4 md:py-5" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.08) transparent", scrollBehavior: "smooth", WebkitOverflowScrolling: "touch" }}>
        <div className="flex gap-2 md:gap-3 min-w-max h-full pb-3">
          {COLUMNS.map((col) => {
  ...
  return (
    <div className="px-3 md:px-5 py-4 md:py-6">
      ...
      <div className="flex items-center justify-between mb-4 md:mb-6">
        <div>
          <h1 className="text-white font-bold text-lg md:text-xl tracking-tight">System Design Studio</h1>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center rounded-xl p-0.5 gap-0.5" style={G.btn}>
            <button onClick={() => setStudioView("projects")}
              className={clsx("h-7 px-3 rounded-lg text-[11px] md:text-[12px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform",
                studioView === "projects" ? "text-white" : "text-[#484f58] hover:text-[#8b949e]")}
              style={studioView === "projects" ? { background: "rgba(255,255,255,0.12)", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.10)" } : undefined}>
              Projects
            </button>
            <button onClick={() => setStudioView("canvas")}
              className={clsx("h-7 px-3 rounded-lg text-[11px] md:text-[12px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform",
                studioView === "canvas" ? "text-white" : "text-[#484f58] hover:text-[#8b949e]")}
              style={studioView === "canvas" ? { background: "rgba(255,255,255,0.12)", boxShadow: "inset 0 1px 0 rgba(255,255,255,0.10)" } : undefined}>
              Canvas
            </button>
          </div>
          {studioView === "projects" && (
            <>
              ...
              <button onClick={() => setShowUploadModal(true)}
                className="flex items-center gap-1.5 h-8 px-3 md:px-4 rounded-xl text-white text-[11px] md:text-[12px] font-bold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
                style={{ background: "#3b82f6", boxShadow: "0 4px 16px rgba(59,130,246,0.35), inset 0 1px 0 rgba(255,255,255,0.2)" }}>
      ...
      {studioView === "canvas" ? (
        <div className="rounded-2xl overflow-hidden" style={{ ...G.card, minHeight: "60vh" }}>
          <div className="relative w-full" style={{ paddingBottom: "56.25%" }}>
            <svg viewBox="0 0 990 610" className="absolute inset-0 w-full h-full" xmlns="http://www.w3.org/2000/svg">
              <rect width="990" height="610" fill="#070c1a" />
              <defs>
                <pattern id="cgds" width="30" height="30" patternUnits="userSpaceOnUse"><path d="M 30 0 L 0 0 0 30" fill="none" stroke="rgba(255,255,255,0.024)" strokeWidth="0.5" /></pattern>
              </defs>
              ...
              <circle cx="870" cy="530" r="5" fill="#3b82f6" />
              <path d={fovPath(870, 530, 315, 80, 110)} fill="rgba(59,130,246,0.18)" />
              <circle cx="518" cy="270" r="5" fill="#3b82f6" />
              <path d={fovPath(518, 270, 135, 80, 110)} fill="rgba(59,130,246,0.18)" />
            </svg>
          </div>
          <div className="absolute bottom-4 left-4 right-4 flex items-center justify-between pointer-events-none">
            <div className="px-3 py-1.5 rounded-xl text-[10px] md:text-[11px] font-semibold text-white pointer-events-auto" style={G.liquidGlass}>
              Casino — Floor Surveillance · Rev C
            </div>
            <button onClick={() => navigate("design-canvas")} className="px-3 py-1.5 rounded-xl text-[10px] md:text-[11px] font-bold text-white pointer-events-auto cursor-pointer active:scale-[0.97] transition-transform" style={{ background: "#3b82f6" }}>
              Open Full Canvas
            </button>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 mb-4 md:mb-5 flex-wrap">
            {stageFilters.map((f) => (
              <button key={f.id} onClick={() => setFilter(f.id)}
                className={clsx("h-7 px-3 rounded-full text-[11px] md:text-[12px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform",
            ...
            <div className="relative ml-1">
              ...
              <input
                ...
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search projects…"
                className="h-7 rounded-xl pl-7 pr-3 text-[11px] md:text-[12px] text-[#e6edf3] placeholder:text-[#484f58] focus:outline-none focus:ring-1 focus:ring-blue-500/50 w-36 md:w-44 transition-all"
                style={G.input} />
            </div>
          ...
          {viewMode === "grid" ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 md:gap-4">
              {filtered.map((project) => {
                ...
                const hasDesign = ["design", "proposal", "negotiation", "win"].includes(project.stage);
                return (
                  <div key={project.id} className="group rounded-2xl overflow-hidden cursor-pointer transition-all duration-200 md:hover:-translate-y-1 relative"
                    style={{ ...G.card }}>
                    ...
                    <div className="p-3 md:p-4" onClick={() => navigate("project-detail")}>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2 md:gap-2.5">
                          <span className="flex items-center gap-1 text-[#484f58] text-[10px] md:text-[11px]"><Camera className="w-3 h-3" />{project.cameras}</span>
                          <span className="flex items-center gap-1 text-[#484f58] text-[10px] md:text-[11px]"><Fingerprint className="w-3 h-3" />{project.devices}</span>
                        </div>
                        <div className="flex items-center gap-1.5">
            ...
            <div className="rounded-2xl overflow-hidden" style={G.card}>
              <div className="overflow-x-auto">
                <div className="grid gap-3 md:gap-4 px-3 md:px-4 py-2.5" style={{ gridTemplateColumns: "2fr 1fr 80px 80px 100px 80px 32px", minWidth: "700px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
                ...
                {filtered.map((project) => {
                  ...
                  return (
                    <div key={project.id}
                      className="grid gap-3 md:gap-4 px-3 md:px-4 py-3.5 items-center transition-all group hover:bg-white/[0.03]"
                      style={{ gridTemplateColumns: "2fr 1fr 80px 80px 100px 80px 32px", minWidth: "700px", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                      <div className="min-w-0 cursor-pointer" onClick={() => navigate("project-detail")}>
                        <p className="text-white text-[12px] md:text-[13px] font-semibold truncate group-hover:text-blue-400 transition-colors">{project.name}</p>
                        <p className="text-[#484f58] text-[10px] md:text-[11px] truncate">{project.location}</p>
                      </div>
    ...
    <div className="px-3 md:px-5 py-4 md:py-6 max-w-[1200px]">
      <div className="flex flex-col md:flex-row md:items-start justify-between mb-4 md:mb-6 gap-4">
        <div className="min-w-0">
          ...
          <p className="text-[#8b949e] text-[12px] md:text-[13px] flex items-center gap-1.5">
        ...
        <div className="flex items-center gap-2 flex-shrink-0 flex-wrap">
          {[
            ...
            <button key={label} onClick={action}
              className="flex items-center gap-1.5 h-9 px-3 md:px-4 rounded-xl text-white text-[11px] md:text-[12px] font-semibold transition-all hover:bg-white/[0.10] cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
              style={G.btn}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-3 mb-4 md:mb-6">
        {[
          ...
          <div key={s.label} className="rounded-2xl p-3 md:p-4" style={G.card}>
            <div className="flex items-center justify-between mb-2 md:mb-3">
              <span className="text-[#8b949e] text-[9px] md:text-[10px] font-bold uppercase tracking-[0.06em] md:tracking-[0.09em]">{s.label}</span>
              <div className="w-6 h-6 md:w-7 md:h-7 rounded-xl flex items-center justify-center"
                style={{ background: `${s.color}18`, border: `1px solid ${s.color}30` }}>
                <s.icon className="w-3 h-3 md:w-3.5 md:h-3.5" style={{ color: s.color }} />
              </div>
            </div>
            <p className="text-white text-lg md:text-xl font-bold tracking-tight">{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center gap-0.5 mb-4 md:mb-5 overflow-x-auto" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)", scrollbarWidth: "none" }}>
        {tabs.map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={clsx("h-10 px-3 md:px-4 text-[12px] md:text-[13px] font-semibold border-b-2 transition-all -mb-px whitespace-nowrap cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]",
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="md:col-span-2 space-y-4">
            <div className="rounded-2xl p-4 md:p-5" style={G.card}>
              <h3 className="text-white text-[13px] md:text-[14px] font-bold mb-3">Project Scope</h3>
              <p className="text-[#8b949e] text-[12px] md:text-[13px] leading-relaxed">{p.summary ?? "Full-coverage surveillance deployment across the main casino floor, VIP suites, cage operations, and surrounding perimeter."}</p>
            </div>
            <div className="rounded-2xl p-4 md:p-5" style={G.card}>
              <h3 className="text-white text-[13px] md:text-[14px] font-bold mb-4">Project Team</h3>
              <div className="space-y-3">
                {[{ name: p.assignee.name, role: "Account Manager", initials: p.assignee.initials, color: p.assignee.color },
                  ...
                  <div key={m.name} className="flex items-center gap-3">
                    ...
                    <div><p className="text-white text-[12px] md:text-[13px] font-semibold">{m.name}</p><p className="text-[#8b949e] text-[10px] md:text-[11px]">{m.role}</p></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="space-y-4">
                ...
                {[
                  ...
                  <div key={item.phase} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 flex items-center justify-between">
                      <span className={clsx("text-[11px] md:text-[12px] font-semibold", item.done ? "text-[#8b949e]" : item.current ? "text-white" : "text-[#484f58]")}>{item.phase}</span>
                      <span className="text-[#484f58] text-[10px] md:text-[11px]">{item.date}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rounded-2xl p-4 md:p-5" style={G.card}>
              <h3 className="text-white text-[13px] md:text-[14px] font-bold mb-3">Quick Links</h3>
              <div className="space-y-1.5">
                {[
                  ...
                  <button key={link.label} className="w-full flex items-center gap-2.5 h-8 px-3 rounded-xl hover:bg-white/[0.05] transition-colors text-left cursor-pointer active:scale-[0.97] transition-transform">
                    ...
                    <span className="text-[#8b949e] text-[11px] md:text-[12px] font-medium hover:text-white transition-colors">{link.label}</span>
                  </button>
                ))}
              </div>
            </div>
      ...
      {activeTab === "quotes" && (
        <div className="space-y-3">
          {[
            ...
            <div key={q.id} className="flex items-center justify-between rounded-2xl p-4 transition-all group md:hover:-translate-y-0.5" style={G.card}>
              ...
              <div className="flex items-center gap-4">
                ...
                <button onClick={() => navigate("quote-builder")}
                  className="h-8 px-3 rounded-xl text-[#8b949e] text-[12px] font-semibold hover:text-white transition-all opacity-0 group-hover:opacity-100 cursor-pointer active:scale-[0.97] transition-transform hidden md:block"
      ...
      {activeTab === "audit-log" && (
        <div className="rounded-2xl overflow-hidden" style={G.card}>
          {AUDIT_LOG.map((entry) => {
            ...
            return (
              <div key={entry.id} className="flex items-center gap-4 px-4 md:px-5 py-3 md:py-4 hover:bg-white/[0.02] transition-colors"
                style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-white text-[12px] md:text-[13px] font-semibold">{entry.action}</p>
                  <p className="text-[#484f58] text-[10px] md:text-[11px]">{entry.user}</p>
                </div>
                <span className="text-[#484f58] text-[10px] md:text-[11px] flex-shrink-0">{entry.time}</span>
              </div>
            );
          })}
        </div>
...
function DesignCanvas({ navigate }: { navigate: (p: Page) => void }) {
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: "#070c1a" }}>
      <header className="h-12 flex items-center gap-3 md:gap-4 px-3 md:px-4 flex-shrink-0 z-40" style={G.liquidGlass}>
        <button onClick={() => navigate("project-detail")} className="flex items-center gap-1.5 text-[#8b949e] hover:text-white text-[11px] md:text-[12px] font-semibold transition-colors flex-shrink-0 cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]">
          <ArrowLeft className="w-3.5 h-3.5" /> Back
        </button>
        <div className="w-px h-4 flex-shrink-0 hidden md:block" style={{ background: "rgba(255,255,255,0.10)" }} />
        ...
        <button onClick={() => setShowFov(!showFov)}
          className={clsx("flex items-center gap-1.5 h-7 px-2 md:px-3 rounded-xl text-[10px] md:text-[11px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform",
          ...
          <Eye className="w-3 h-3" /> <span className="hidden md:inline">Show FOV</span>
        </button>
        <button onClick={() => setShowDeviceTray(!showDeviceTray)}
          className={clsx("flex items-center gap-1.5 h-7 px-2 md:px-3 rounded-xl text-[10px] md:text-[11px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform",
            showDeviceTray ? "text-white" : "text-[#8b949e] hover:text-white")}
          style={G.btn}>
          <Package className="w-3 h-3" /> <span className="hidden md:inline">Devices</span>
        </button>
        <button className="flex items-center gap-1.5 h-7 px-2 md:px-3 rounded-xl text-[#e6edf3] text-[10px] md:text-[11px] font-semibold hover:bg-white/[0.10] transition-all cursor-pointer active:scale-[0.97] transition-transform" style={G.btn}>
          <Download className="w-3 h-3" /> <span className="hidden md:inline">Export PDF</span>
        </button>
        <button className="flex items-center gap-1.5 h-7 px-2 md:px-3 rounded-xl text-white text-[10px] md:text-[11px] font-bold cursor-pointer active:scale-[0.97] transition-transform"
          style={{ background: "#3b82f6", boxShadow: "0 2px 12px rgba(59,130,246,0.35)", border: "1px solid rgba(255,255,255,0.15)" }}>
          <Share2 className="w-3 h-3" /> <span className="hidden md:inline">Share</span>
        </button>
      </header>
      ...
      <div className="flex-1 relative overflow-hidden">
        <motion.div
          ...
          <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            ...
            <button onClick={() => setShowDeviceTray(false)} className="w-6 h-6 rounded-lg hover:bg-white/[0.08] flex items-center justify-center cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px]">
        ...
        {showProperties && selected && (
          <div className="absolute right-0 top-0 bottom-0 w-72 z-30 flex flex-col" style={G.liquidGlass}>
            <div className="px-4 py-3 flex items-center justify-between" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              ...
              <button onClick={() => setShowProperties(false)} className="w-6 h-6 rounded-lg hover:bg-white/[0.08] flex items-center justify-center cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px]">
                <X className="w-3.5 h-3.5 text-[#8b949e]" />
              </button>
            </div>
        ...
        <div className="absolute bottom-5 left-1/2 -translate-x-1/2 z-40 flex items-center gap-1 px-2 md:px-3 py-2 rounded-2xl" style={G.liquidGlass}>
          {CANVAS_TOOLS.map((tool, i) => (
            <div key={tool.id} className="flex items-center">
              ...
              <button onClick={() => setActiveTool(tool.id)} title={tool.label}
                className={clsx("w-8 h-8 md:w-9 md:h-9 rounded-xl flex items-center justify-center transition-all duration-150 cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0",
          ...
          <button onClick={() => setShowProperties(!showProperties)}
            className={clsx("w-8 h-8 md:w-9 md:h-9 rounded-xl flex items-center justify-center transition-all cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px] md:min-w-0 md:min-h-0",
...
function QuoteBuilder({ navigate, quoteItems, setQuoteItems }: { navigate: (p: Page) => void; quoteItems: QuoteItem[]; setQuoteItems: React.Dispatch<React.SetStateAction<QuoteItem[]>> }) {
  ...
  return (
    <div className="flex flex-col md:flex-row h-[calc(100vh-56px)] overflow-hidden">
      ...
      <div className="hidden md:flex w-72 flex-shrink-0 flex-col" style={{ ...sidebarStyle, borderRight: "1px solid rgba(255,255,255,0.07)" }}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <div className="px-4 md:px-6 py-4 md:py-5 flex flex-col md:flex-row md:items-center justify-between flex-shrink-0 gap-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          <div>
            <div className="flex items-center gap-2 mb-1">
              <h2 className="text-white font-bold text-[14px] md:text-[15px]">Q-2026-044-v3</h2>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-1.5 text-[#8b949e] hover:text-white text-[11px] md:text-[12px] font-semibold transition-colors cursor-pointer active:scale-[0.97] transition-transform"
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => { downloadPDF(`Q-2026-044-v3_${selectedProject?.client?.replace(/[^a-z0-9]/gi,"_") ?? "quote"}.pdf`, items); }}
              className="flex items-center gap-1.5 h-8 px-3 rounded-xl text-[#8b949e] text-[10px] md:text-[11px] font-semibold hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform" style={G.btn}>
              <FileText className="w-3.5 h-3.5" /> Export PDF
            </button>
            <button onClick={() => toast.success("Quote Q-2026-044-v3 approved")}
              className="flex items-center gap-1.5 h-8 px-3 md:px-4 rounded-xl text-white text-[10px] md:text-[11px] font-bold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
              style={{ background: "#3b82f6", boxShadow: "0 4px 16px rgba(59,130,246,0.35), inset 0 1px 0 rgba(255,255,255,0.2)" }}>
        ...
        <div className="hidden md:grid gap-4 px-5 py-3 flex-shrink-0" style={{ gridTemplateColumns: "2fr 110px 130px 130px 32px", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "rgba(255,255,255,0.02)" }}>
        ...
        <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: "thin", scrollbarColor: "rgba(255,255,255,0.08) transparent" }}>
          {items.map((item, idx) => {
            ...
            return (
              <div key={item.id}
                ...
                className={clsx("grid gap-2 md:gap-4 px-3 md:px-5 py-3 md:py-4 items-center cursor-pointer transition-colors group",
                ...
                <div className="min-w-0">
                  <p className="text-white text-[11px] md:text-[12px] font-semibold truncate">{item.name}</p>
                  <p className="text-[#484f58] text-[10px] tabular-nums">Unit: {fmt(item.unitPrice)}</p>
                </div>
                <span className="text-[#484f58] text-[10px] md:text-[11px] font-mono truncate">{item.sku}</span>
                <div className="flex items-center gap-1">
                  ...
                  <span className="text-white text-[11px] md:text-[12px] font-semibold w-10 text-center tabular-nums">{item.qty}</span>
                ...
                <span className="text-white text-[12px] md:text-[13px] font-bold tabular-nums">{fmt(lineTotal)}</span>
          ...
          <div className="px-3 md:px-5 py-3">
            <button className="flex items-center gap-2 text-[#484f58] hover:text-blue-400 text-[11px] md:text-[12px] font-semibold transition-colors cursor-pointer active:scale-[0.97] transition-transform">
      ...
      <div className="hidden md:flex w-72 flex-shrink-0 flex-col" style={{ ...sidebarStyle, borderLeft: "1px solid rgba(255,255,255,0.07)" }}>
        <div className="px-4 py-3" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
...
const CATALOG_DEVICES: CatalogDevice[] = [
  ...
  { id: "n3", model: "AutoVu ALPR 6.0", manufacturer: "Genetec", category: "analytics", sku: "GSC-ALPR", price: 8500, channels: "License plate recognition", imageUrl: IMG.outdoor, tags: ["LPR"] },
    ...
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <motion.div initial={{ opacity: 0, scale: 0.93, y: 18 }} animate={{ opacity: 1, scale: 1, y: 0 }}
        ...
        <div className="w-full md:w-56 flex-shrink-0 relative" style={{ background: "rgba(255,255,255,0.03)", minHeight: "200px", maxHeight: "300px", md: { minHeight: "320px", maxHeight: "none" } }}>
          ...
          <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(7,12,26,0.8), transparent 60%)" }} />
          <div className="absolute bottom-4 left-4 flex flex-wrap gap-1.5">
            ...
            {device.tags?.map((tag) => {
              ...
              return ts ? (
                <span key={tag} className="inline-block px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase tracking-wider" style={{ background: ts.bg, color: ts.text, border: `1px solid ${ts.border}` }}>
                  {tag}
                </span>
              ) : null;
            })}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <div className="px-5 md:px-6 pt-5 md:pt-6 pb-4 flex items-start justify-between flex-shrink-0" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
            <div>
              ...
              {device.price && (
                <p className="text-[0.9rem] md:text-[1rem] font-bold mt-1 tabular-nums" style={{ color: cc.text }}>{fmt(device.price)} <span className="text-[#484f58] text-[10px] md:text-[11px] font-normal">/ unit (excl. GCT)</span></p>
              )}
            </div>
            <button onClick={onClose} className="w-8 h-8 rounded-xl flex items-center justify-center hover:bg-white/[0.08] transition-colors flex-shrink-0 cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px]"
              style={{ border: "1px solid rgba(255,255,255,0.10)" }}>
              <X className="w-4 h-4 text-[#8b949e]" />
            </button>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3">
              {specs.map((s) => (
                <div key={s.label} className="rounded-xl p-3" style={G.subtle}>
                  ...
                  <p className="text-white text-[11px] md:text-[12px] font-semibold">{s.value}</p>
                </div>
              ))}
            </div>
          </div>
          <div className="px-5 md:px-6 py-4 flex items-center gap-3 flex-shrink-0" style={{ borderTop: "1px solid rgba(255,255,255,0.07)" }}>
            <button onClick={() => { addToQuote(device); onClose(); }}
              className="flex items-center gap-2 h-9 px-4 md:px-5 rounded-xl text-white text-[11px] md:text-[12px] font-bold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
              style={{ background: "#3b82f6", boxShadow: "0 4px 16px rgba(59,130,246,0.35), inset 0 1px 0 rgba(255,255,255,0.2)" }}>
              <Plus className="w-3.5 h-3.5" /> Add to Quote
            </button>
            <button onClick={() => toast.success(`Spec sheet for ${device.model} would open here`)}
              className="flex items-center gap-2 h-9 px-3 md:px-4 rounded-xl text-[#8b949e] text-[11px] md:text-[12px] font-semibold hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform"
              style={G.btn}>
...
function DeviceStore({ navigate: _navigate }: { navigate: (p: Page) => void }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (search.trim()) {
      const q = search.toLowerCase();
      result = result.filter((d) => d.model.toLowerCase().includes(q) || d.manufacturer.toLowerCase().includes(q) || (d.sku ?? "").toLowerCase().includes(q) || (d.tags ?? []).some(t => t.toLowerCase().includes(q)));
    }
    return result;
  ...
  return (
    <div className="px-3 md:px-5 py-4 md:py-6">
      ...
      <div className="flex items-center justify-between mb-4 md:mb-6">
        <div>
          <h1 className="text-white font-bold text-lg md:text-xl tracking-tight">Device Store</h1>
          <p className="text-[#8b949e] text-[11px] md:text-[13px] mt-0.5">{filtered.length} products · Axis · Avigilon · Pelco · Genetec · Suprema · HID</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex items-center h-8 rounded-xl overflow-hidden" style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.10)" }}>
      ...
      <div className="mb-4 md:mb-5 flex items-center gap-2 md:gap-3 flex-wrap">
        <div className="relative flex-1 min-w-[160px] max-w-sm">
          ...
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search model, SKU, tags…" className={`${inputCls} pl-9 w-full`} style={G.input} />
        </div>
        <div className="flex gap-1.5 md:gap-2 flex-wrap">
          {categories.map((c) => (
            <button key={c.id} onClick={() => setCategoryFilter(c.id)}
              className="h-8 md:h-9 px-2.5 md:px-3.5 rounded-xl text-[11px] md:text-[12px] font-semibold transition-all cursor-pointer active:scale-[0.97] transition-transform whitespace-nowrap"
        ...
        <div className="relative">
          <select value={manufacturerFilter} onChange={(e) => setManufacturerFilter(e.target.value)}
            className={`${inputCls} appearance-none cursor-pointer pr-7 min-w-[130px]`} style={G.input}>
          ...
          {filtered.map((device) => {
            ...
            return (
              <div key={device.id} onClick={() => setSelectedDevice(device)}
                className="rounded-2xl overflow-hidden cursor-pointer group transition-all md:hover:-translate-y-1"
                ...
                <div className="p-3 md:p-3.5">
                  ...
                  {device.authentication && <p className="text-[#484f58] text-[9px] md:text-[10px] mt-1 truncate">{device.authentication}</p>}
                  {device.channels && !device.resolution && <p className="text-[#484f58] text-[9px] md:text-[10px] mt-1 truncate">{device.channels}</p>}
                  {device.tags && device.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {device.tags.map((tag) => {
                        const ts = TAG_STYLES[tag];
                        return ts ? (
                          <span key={tag} className="inline-block px-1.5 py-0.5 rounded-md text-[8px] font-bold uppercase tracking-wider" style={{ background: ts.bg, color: ts.text, border: `1px solid ${ts.border}` }}>
                            {tag}
                          </span>
                        ) : null;
                      })}
                  ...
                  <div className="flex items-center justify-between mt-2 md:mt-3 pt-2 md:pt-2.5" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                    <span className="text-[#484f58] text-[8px] md:text-[9px] font-mono">{device.sku}</span>
                    <span className="font-bold text-[11px] md:text-[12px] tabular-nums" style={{ color: cc.text }}>{device.price ? fmt(device.price) : "—"}</span>
                  </div>
                </div>
              </div>
            );
            ...
            <table className="w-full" style={{ minWidth: "900px" }}>
              <thead>
                <tr style={{ background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
                  {["", "Model", "Manufacturer", "Category", "Resolution", "Tags", "SKU", "Price"].map((h) => (
                    <th key={h} className={`${h === "Price" ? "text-right" : "text-left"} px-3 md:px-4 py-3 text-[#484f58] text-[10px] font-bold uppercase tracking-widest`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {filtered.map((device, i) => {
                  ...
                  return (
                    <tr key={device.id} onClick={() => setSelectedDevice(device)} className="cursor-pointer hover:bg-white/[0.02] transition-colors group"
                      ...
                      <td className="px-3 md:px-4 py-2.5">
                        <span className="inline-block px-2 py-0.5 rounded-lg text-[9px] md:text-[10px] font-bold uppercase tracking-wider" style={{ background: cc.bg, color: cc.text }}>{cc.label}</span>
                      </td>
                      ...
                      <td className="px-3 md:px-4 py-2.5 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <span className="text-white text-[11px] md:text-[12px] font-bold tabular-nums">{device.price ? fmt(device.price) : "—"}</span>
                          <button onClick={(e) => { e.stopPropagation(); addToQuote(device); }}
  ...
  return (
    <div className="px-3 md:px-5 py-4 md:py-6 max-w-[1100px]">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 md:mb-6 gap-3">
        <div>
          <h1 className="text-white font-bold text-lg md:text-xl tracking-tight">Install Tracker</h1>
          <p className="text-[#8b949e] text-[11px] md:text-[13px] mt-0.5">Data Center — Full Security Stack · Stratum Cloud Services</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={handleExport} className="flex items-center gap-1.5 h-8 px-3 rounded-xl text-[#8b949e] text-[11px] md:text-[12px] font-semibold hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform" style={G.btn}>
            <Download className="w-3.5 h-3.5" /> Export Report
          </button>
          <button onClick={() => setShowAddDevice(true)} className="flex items-center gap-1.5 h-8 px-3 md:px-4 rounded-xl text-white text-[11px] md:text-[12px] font-bold cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
            style={{ background: "#3b82f6", boxShadow: "0 4px 16px rgba(59,130,246,0.35), inset 0 1px 0 rgba(255,255,255,0.2)" }}>
      ...
      {showAddDevice && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={() => setShowAddDevice(false)}>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.94, y: 14 }} animate={{ opacity: 1, scale: 1, y: 0 }}
            transition={{ type: "spring", damping: 26, stiffness: 360 }}
            onClick={(e) => e.stopPropagation()}
            className="relative z-10 w-full max-w-[440px] max-h-[90vh] overflow-y-auto rounded-3xl overflow-hidden"
            ...
            <div className="absolute inset-x-0 top-0 h-[2px] rounded-t-3xl" style={{ background: "linear-gradient(90deg, #3b82f6dd, #3b82f633)" }} />
            <div className="flex items-center justify-between px-5 md:px-6 pt-5 md:pt-6 pb-5" style={{ borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
              <div><h2 className="text-white text-[1rem] font-bold">Add Device</h2><p className="text-[#8b949e] text-[11px] md:text-[12px] mt-0.5">Add a new device to the install list</p></div>
              <button onClick={() => setShowAddDevice(false)} className="w-7 h-7 rounded-xl flex items-center justify-center hover:bg-white/[0.08] cursor-pointer active:scale-[0.97] transition-transform min-w-[44px] min-h-[44px]" style={{ border: "1px solid rgba(255,255,255,0.10)" }}><X className="w-4 h-4 text-[#8b949e]" /></button>
            </div>
            <form onSubmit={handleAddDevice} className="px-5 md:px-6 py-5 space-y-4">
              ...
              <div className="flex gap-3 pt-1">
                <button type="button" onClick={() => setShowAddDevice(false)}
                  className="flex-1 h-10 rounded-xl text-[#8b949e] text-[13px] font-semibold hover:text-white transition-all cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]" style={G.btn}>Cancel</button>
                <button type="submit" disabled={!newDeviceName.trim()}
                  className="flex-1 h-10 rounded-xl text-white text-[13px] font-bold disabled:opacity-40 cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
      ...
      <div className="rounded-2xl p-4 md:p-5 mb-4 md:mb-5" style={G.card}>
        <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 gap-3">
          <div>
            <p className="text-[#8b949e] text-[10px] md:text-[11px] font-bold uppercase tracking-widest mb-1">Overall Completion</p>
            <p className="text-white text-[1.6rem] md:text-[2rem] font-bold tracking-tight leading-none">{pct}%</p>
          </div>
          <div className="grid grid-cols-4 gap-3 md:gap-4 text-right">
            {[
              ...
              <div key={s.label}>
                <p className={clsx("text-[1.1rem] md:text-[1.3rem] font-bold leading-none", s.color)}>{s.count}</p>
                <p className="text-[#484f58] text-[9px] md:text-[10px] font-semibold mt-0.5">{s.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-4 mt-2">
          <span className="text-[#8b949e] text-[10px] md:text-[11px]">{complete} of {total} devices installed</span>
          {failed > 0 && <span className="text-rose-400 text-[10px] md:text-[11px] font-semibold flex items-center gap-1"><AlertTriangle className="w-3 h-3" />{failed} defect{failed > 1 ? "s" : ""}</span>}
        </div>
      </div>
          ...
          return (
            <div key={zone.id} className="rounded-2xl overflow-hidden" style={G.card}>
              <button onClick={() => setExpandedZone(isExpanded ? null : zone.id)}
                className="w-full flex items-center gap-3 md:gap-4 px-4 md:px-5 py-3 md:py-4 hover:bg-white/[0.02] transition-colors cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]">
                <div className="flex-1 flex items-center gap-3 md:gap-4 min-w-0">
                  <div className={clsx("w-7 h-7 md:w-8 md:h-8 rounded-xl flex items-center justify-center flex-shrink-0",
                    ...
                    {zPct === 100 ? <CheckCircle2 className="w-3.5 h-3.5 md:w-4 md:h-4 text-emerald-400" /> : zPct > 0 ? <Clock className="w-3.5 h-3.5 md:w-4 md:h-4 text-blue-400" /> : <AlertCircle className="w-3.5 h-3.5 md:w-4 md:h-4 text-[#484f58]" />}
                  </div>
                  <div className="min-w-0">
                    <p className="text-white text-[12px] md:text-[13px] font-bold text-left">{zone.name}</p>
                    <p className="text-[#484f58] text-[10px] md:text-[11px] text-left">{zone.devices.length} devices · {zComplete} complete</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 md:gap-4 flex-shrink-0">
                  <div className="w-20 md:w-24 h-1.5 rounded-full overflow-hidden" style={{ background: "rgba(255,255,255,0.07)", boxShadow: "inset 0 1px 1px rgba(0,0,0,0.3)" }}>
                    <div className={clsx("h-full rounded-full transition-all", zPct === 100 ? "bg-emerald-500" : "bg-blue-500")} style={{ width: `${zPct}%` }} />
                  </div>
                  <span className={clsx("text-[11px] md:text-[12px] font-bold w-10 text-right", zPct === 100 ? "text-emerald-400" : "text-white")}>{zPct}%</span>
                  {isExpanded ? <ChevronUp className="w-4 h-4 text-[#484f58]" /> : <ChevronDown className="w-4 h-4 text-[#484f58]" />}
                </div>
              </button>
              {isExpanded && (
                <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
                  <div className="hidden md:grid gap-4 px-5 py-2.5" style={{ gridTemplateColumns: "36px 2fr 1.5fr 120px 1fr", background: "rgba(255,255,255,0.02)" }}>
                  ...
                  {zone.devices.map((device, i) => {
                    ...
                    return (
                      <div key={device.id}
                        className={clsx("grid gap-2 md:gap-4 px-3 md:px-5 py-3 md:py-3.5 items-center hover:bg-white/[0.015] transition-colors", i % 2 === 1 && "bg-white/[0.01]")}
                        ...
                        <div className="min-w-0">
                          <p className="text-white text-[11px] md:text-[12px] font-semibold truncate">{device.name}</p>
                          {device.notes && <p className="text-rose-400 text-[9px] md:text-[10px] flex items-center gap-1 mt-0.5 truncate"><AlertTriangle className="w-2.5 h-2.5 flex-shrink-0" /> {device.notes}</p>}
                        </div>
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-[#8b949e] text-[10px] md:text-[11px] truncate">{device.assignee}</span>
                        </div>
                        <div className="relative">
                          <select value={device.status} onChange={(e) => updateStatus(zone.id, device.id, e.target.value as InstallStatus)}
                            className={clsx("w-full h-7 rounded-xl border px-2 text-[10px] md:text-[11px] font-bold appearance-none cursor-pointer transition-all focus:outline-none pr-6", meta.bg, meta.color, "border-transparent hover:border-white/[0.10]")}
...
function LoginPage({ onLogin }: { onLogin: () => void }) {
  ...
  return (
    <div className="h-screen flex overflow-hidden">
      <div className="hidden lg:flex w-[48%] flex-shrink-0 flex-col relative overflow-hidden" style={{ background: "#070c1a" }}>
        ...
        <div className="relative z-10 flex flex-col h-full p-8 md:p-12">
          <div className="mb-auto">
            <img src={logoImg} alt="E-Tech Systems" className="h-8 md:h-10 object-contain object-left" style={{ filter: "brightness(1.1)" }} />
          </div>
          ...
          <div className="flex flex-col justify-center flex-1 py-8 md:py-12">
            ...
            <h1 className="text-white text-[2rem] md:text-[2.6rem] font-bold leading-[1.12] tracking-tight mb-4 md:mb-6">
            ...
            <p className="text-[#8b949e] text-[11px] md:text-[13px] leading-relaxed mb-8 md:mb-10 max-w-[320px] md:max-w-[380px]">
            ...
            <div className="space-y-2 md:space-y-3">
              {[
                ...
                <div key={title} className="flex items-start gap-3 md:gap-4 p-3 md:p-4 rounded-2xl transition-all md:hover:-translate-y-0.5"
                  ...
                  <div>
                    <p className="text-white text-[12px] md:text-[13px] font-bold mb-0.5">{title}</p>
                    <p className="text-[#8b949e] text-[11px] md:text-[12px] leading-relaxed">{desc}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 md:p-8 relative" style={{ background: "rgba(255,255,255,0.03)", backdropFilter: "blur(40px)", WebkitBackdropFilter: "blur(40px)" }}>
        ...
        >
          <div className="lg:hidden mb-8">
            <img src={logoImg} alt="E-Tech Systems" className="h-8 object-contain object-left" style={{ filter: "brightness(1.1)" }} />
          </div>
          ...
          <div className="rounded-3xl p-6 md:p-8"
            ...
            <button onClick={submit}
              className="w-full flex items-center justify-center gap-3 h-11 rounded-2xl text-white text-[12px] md:text-[13px] font-bold transition-all duration-150 mb-5 md:mb-6 hover:bg-white/[0.12] active:scale-[0.99] cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
            ...
            <div className="flex items-center gap-3 mb-5 md:mb-6">
              ...
              <span className="text-[#484f58] text-[10px] md:text-[11px] font-semibold">or continue with email</span>
              <div className="flex-1 h-px" style={{ background: "rgba(255,255,255,0.08)" }} />
            </div>

            <form onSubmit={submit} className="space-y-3 md:space-y-4">
              <div>
                <label className="block text-[#e6edf3] text-[10px] md:text-[11px] font-bold mb-1.5 uppercase tracking-[0.07em]">Email</label>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-[#e6edf3] text-[10px] md:text-[11px] font-bold uppercase tracking-[0.07em]">Password</label>
                  <button type="button" className="text-blue-400 text-[11px] md:text-[12px] font-semibold hover:text-blue-300 transition-colors">Forgot password?</button>
                </div>
                <div className="relative">
              ...
              <button type="submit" disabled={loading}
                className="w-full h-11 rounded-2xl text-white font-bold text-[12px] md:text-[13px] transition-all duration-150 active:scale-[0.99] flex items-center justify-center gap-2 disabled:opacity-60 mt-1 cursor-pointer active:scale-[0.97] transition-transform min-h-[44px]"
            ...
            <p className="text-center text-[#484f58] text-[11px] md:text-[12px] mt-5 md:mt-6">
              Need access?{" "}
        ...
        <div className="min-h-screen bg-background">
          ...
          <div className="pt-14">
            <AnimatePresence mode="wait">
              ...
              {page === "quote-builder" && <QuoteBuilder key="quote-builder" navigate={setPage} quoteItems={quoteItems} setQuoteItems={setQuoteItems} />}
